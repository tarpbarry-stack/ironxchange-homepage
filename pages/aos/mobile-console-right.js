...
export default function MobileConsoleRightCertificationPage() {
  ...
  function updateIxiCardState(id, patch) {
    setIxiCardState(current => {
      ...
      if (Array.isArray(nextPatch.consoleSlots) && nextPatch.consoleSlots.length > 2) {
        nextPatch = { ...nextPatch, consoleSlots: nextPatch.consoleSlots.slice(0, 2) };
      }
  ...
  return (
    <>
      <Head>
        <title>IXI Mobile AOS Right Console Certification</title>
        <meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover" />
      </Head>

      <main className="shell">
        <header className="header">
          <div><strong>IXI MOBILE · AOS · CONSOLE</strong><span>RIGHT SIDE ONLY · EXISTING PRIVATE CONSOLE · IMMUTABLE SURFACE</span></div>
          <div className="mode"><span>{consoleDepth === 2 ? "MACHINE + CONSOLE" : "MACHINE"}</span><span>{nativeWidth}×475 NATIVE ROW</span></div>
        </header>
        ...
        <section className="workspace">
          ...
          {status === "ready" && listing && objectId ? (
            <IXIImmutableScaledSurface nativeWidth={nativeWidth} nativeHeight={475} horizontalPadding={16} className="mobile-right-console-gate">
      ...
      <style jsx global>{`
        .mobile-right-console-gate .ixi-private-console-listing-slot .ixi-object-card-actuator.left{display:none!important}
        .mobile-right-console-gate .ixi-private-console-module-slot .ixi-object-card-actuator.right{display:none!important}
      `}</style>
    </>
  );
}
