...
import { IXI_CONSOLE_SLOT_TYPES } from "../../components/ixi-chassis/IXIObjectConsoleEngine";

function publicDataOf(listing = {}) { return listing?.publicData || listing?.attributes?.publicData || {}; }
function metadataOf(listing = {}) { return listing?.metadata || listing?.attributes?.metadata || {}; }
function getPassportId(listing = {}) {
  const publicData = publicDataOf(listing);
  return String(listing?.passportId || publicData?.passportId || listing?.ixiMedia?.passportId || publicData?.ixiMedia?.passportId || "").trim();
}
function isOwnedPrivateCandidate(listing = {}) {
  ...
  return access === "private" && channel !== "auction" && channel !== "auction-archive" && ownershipRole !== "non-owner" && status !== "archived" && status !== "deleted" && Boolean(getPassportId(listing));
}
function listingIndex(slots = []) {
  return Array.isArray(slots) ? slots.findIndex(slot => slot?.type === IXI_CONSOLE_SLOT_TYPES.LISTING) : -1;
}
export function enforceMobileConsoleInvariant(previousSlots = [], requestedSlots = []) {
  if (!Array.isArray(requestedSlots) || requestedSlots.length <= 2) return requestedSlots;
  const previousListingIndex = listingIndex(previousSlots);
  if (previousSlots.length === 2 && previousListingIndex === 1) return requestedSlots.slice(-2);
  if (previousSlots.length === 2 && previousListingIndex === 0) return requestedSlots.slice(0, 2);
  const requestedListingIndex = listingIndex(requestedSlots);
  return requestedListingIndex <= 0 ? requestedSlots.slice(0, 2) : requestedSlots.slice(-2);
}

export default function MobileConsoleCertificationPage() {
  ...
  const activeListingIndex = listingIndex(slots);
  const consoleDepth = Array.isArray(slots) && slots.length > 1 ? 2 : 1;
  const nativeWidth = consoleDepth === 2 ? 600 : 300;
  const modeLabel = useMemo(() => consoleDepth === 1 ? "MACHINE" : activeListingIndex === 0 ? "MACHINE + CONSOLE" : "CONSOLE + MACHINE", [consoleDepth, activeListingIndex]);

  function updateIxiCardState(id, patch) {
    setIxiCardState(current => {
      const existing = current?.[id] || {};
      let nextPatch = { ...(patch || {}) };
      if (Array.isArray(nextPatch.consoleSlots)) {
        nextPatch = { ...nextPatch, consoleSlots: enforceMobileConsoleInvariant(existing?.consoleSlots || [], nextPatch.consoleSlots) };
      }
  ...
  return <>
    <Head>
      <title>IXI Mobile AOS Console Invariant</title>
      <meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover" />
    </Head>
    <main className="shell">
      <header className="header">
        <div><strong>IXI MOBILE · AOS · CONSOLE</strong><span>ONE SIDE AT A TIME · EXISTING PRIVATE CONSOLE · IMMUTABLE SURFACE</span></div>
        <div className="mode"><span>{modeLabel}</span><span>{nativeWidth}×475 NATIVE ROW</span></div>
      </header>
      <section className="workspace">
        ...
        {status === "ready" && listing && objectId ? (
          <IXIImmutableScaledSurface nativeWidth={nativeWidth} nativeHeight={475} horizontalPadding={16} className="mobile-console-invariant-gate">
            <IXIPrivateObjectConsole
              ...
              renderParentCard={({ consoleDepth: activeConsoleDepth, consoleLeftOpen, consoleRightOpen, onExpandConsoleLeft, onExpandConsoleRight }) => (
                <IXIMachineCard
                  listing={listing}
