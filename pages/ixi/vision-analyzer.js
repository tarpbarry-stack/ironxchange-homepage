...
function getRecommendation(scores) {
  ...
  if (resolutionScore < 38) {
    return {
      lane: "UPSCALE",
      reason: "Low resolution image. Needs enlargement/detail recovery before enhancement."
    };
  }
  ...
  if (compressionScore < 45) {
    return {
      lane: "RESTORE",
      reason: "Heavy compression or blockiness detected. Needs artifact repair before sharpening."
    };
  }
  ...
  if (sharpnessScore < 45) {
    return {
      lane: "CLARITY",
      reason: "Soft image. Needs detail extraction, then controlled refinement."
    };
  }
  ...
  if (exposureScore < 45) {
    return {
      lane: "RECOVER",
      reason: "Exposure is weak. Needs brightness/shadow recovery before polish."
    };
  }
  ...
  if (overallScore >= 72) {
    return {
      lane: "CLEAN",
      reason: "Photo is already usable. Needs only safe marketplace polish."
    };
  }
  ...
  if (contrastScore < 48) {
    return {
      lane: "JULIO",
      reason: "Photo has usable detail but lacks depth, paint richness, and presentation."
    };
  }

  return {
    lane: "IX AUTO",
    reason: "Mixed photo. Use balanced enhancement with light restoration."
  };
}
...
export default function IXVisionAnalyzer() {
  ...
  async function handleFile(file) {
    ...
    try {
      ...
      setAnalysis(result);
      console.log("IX Vision Analysis", result);
    } catch (err) {
      console.error("IX Vision Analyzer failed:", err);
      alert(`Analyzer failed: ${err.message || err}`);
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <>
      <Head>
        <title>IX Vision Analyzer | IronXchange</title>
        ...
        <section className="analyzer-shell">
          <header className="analyzer-header">
            <div>
              ...
              <h1>Analyzer Lab</h1>
              <p>Score the photo before we decide which processing lane should touch it.</p>
            </div>
          ...
          <section className="analyzer-grid">
            <section className="photo-stage">
              {previewUrl ? (
                <img src={previewUrl} alt="Analyzed machine" />
              ) : (
              ...
              {analysis ? (
                <>
                  <section className="recommend-card">
                    ...
                    <h2>{analysis.recommendation.lane}</h2>
                ...
                <div className="empty-analysis">
                  Drop in an auction photo, dealer photo, phone photo, or RB-style compressed web image.
                </div>
              )}
            </aside>
          </section>
