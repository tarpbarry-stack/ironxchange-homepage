...
export default function MyListingsV2() {
  ...
  function handleWorkspaceDragEnd(event) {
    ...
    console.log("IXI MY LISTINGS V2 DND DROP", {
      dragId,
      overId,
      sourceContainer,
      targetContainer,
  ...
  useEffect(() => {
    captureIXEvent("my_listings_v2_viewed", {
      page: "account/my-listings-v2"
    });
  }, []);

  useEffect(() => {
    async function loadMyListingsV2() {
      try {
        ...
        console.error("My Listings V2 load failed:", err);
        setSavedIds([]);
        setListings([]);
      }
    }
...
:global(.active-stack-dropzone.stack-vertical) {
  ...
  grid-template-columns:
    repeat(auto-fill, minmax(285px, 340px));
...
.cards.single-card {
  grid-template-columns: minmax(285px, 340px);
  justify-content: center;
}
