...
const PRIVATE_NATIVE_WIDTH = 300;
const PRIVATE_NATIVE_HEIGHT = 475;
...
async function loadRealOwnedPrivateMachine() {
  ...
  if (!Array.isArray(publicListings) || publicListings.length === 0) {
    throw new Error("No production listings are available for native AOS certification.");
  }
  ...
  for (const authorId of authorIds) {
    try {
      ...
      // Continue probing real production owners. No synthetic machine is allowed.
    }
  }
...
export default function MobileAosNativePage() {
  ...
  return (
    <>
      <Head>
        <title>IXI Mobile AOS Native Desktop Geometry</title>
      ...
      <main className="mobile-shell">
        <header className="header">
          <div>
            ...
            <span>DESKTOP-NATIVE PRIVATE CARD · 1.000×</span>
          </div>
      ...
      <style jsx global>{`
        ...
        /* Mobile AOS contract: preserve the exact desktop owned/private geometry. */
        .native-audit-shell .private-listing-card,
        .mobile-shell .private-listing-card {
          width: ${PRIVATE_NATIVE_WIDTH}px !important;
          min-width: ${PRIVATE_NATIVE_WIDTH}px !important;
          max-width: ${PRIVATE_NATIVE_WIDTH}px !important;
        }

        /* Retire the stale <=850px rule that expands both location fields to 100%. */
        .mobile-shell .owned-private-runtime .private-listing-card .location-row {
          width: auto !important;
          flex-wrap: nowrap !important;
        }
        .mobile-shell .owned-private-runtime .private-listing-card .location-input {
          flex: 0 0 auto !important;
          width: 72px !important;
          max-width: 72px !important;
          text-align: right !important;
        }
        .mobile-shell .owned-private-runtime .private-listing-card .city-input {
          flex: 0 0 76px !important;
          width: 76px !important;
          max-width: 76px !important;
          text-align: right !important;
        }
        .mobile-shell .owned-private-runtime .private-listing-card .state-input {
        ...
        /* Current owned/private Face 1 contract. Lifecycle controls live on Face 2. */
        .mobile-shell .owned-private-runtime .private-listing-card .seller-actions {
      ...
      <style jsx>{`
        ...
        .header span { font-size: 9px; line-height: 1.2; letter-spacing: .45px; color: rgba(255,255,255,.68); }
        .runtime { flex: 0 0 auto; display: grid; justify-items: end; gap: 2px; }
        .runtime span:first-child { color: #7fe1ff; font-weight: 800; }
        .targets {
          box-sizing: border-box;
          width: calc(100% - 16px);
          margin: 0 8px 10px;
        ...
        .targets span { font-size: 9px; color: rgba(255,255,255,.48); }
        .workspace {
          box-sizing: border-box;
          width: 100%;
          min-height: 560px;
        ...
        .native-frame {
          position: relative;
          flex: 0 0 ${PRIVATE_NATIVE_WIDTH}px;
          width: ${PRIVATE_NATIVE_WIDTH}px;
          height: ${PRIVATE_NATIVE_HEIGHT}px;
          min-width: ${PRIVATE_NATIVE_WIDTH}px;
          min-height: ${PRIVATE_NATIVE_HEIGHT}px;
          max-width: ${PRIVATE_NATIVE_WIDTH}px;
          max-height: ${PRIVATE_NATIVE_HEIGHT}px;
          overflow: visible;
          transform: none;
        }
        .message {
