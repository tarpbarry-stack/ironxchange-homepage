...
export default async function handler(req, res) {
  ...
  if (!["GET", "PATCH"].includes(method)) {
    ...
    return res.status(405).json({
      ok: false,
      error: { code: "METHOD_NOT_ALLOWED", message: "GET or PATCH required." }
    });
  }
  ...
  if (!financialDocumentId) {
    return res.status(400).json({
      ok: false,
      error: { code: "FINANCIAL_DOCUMENT_ID_REQUIRED", message: "Financial document ID is required." }
    });
  }
  ...
  return proxyIXIFinancialRequest({
    req,
    res,
    path: `/financial/documents/${encodeURIComponent(financialDocumentId)}`,
    method,
