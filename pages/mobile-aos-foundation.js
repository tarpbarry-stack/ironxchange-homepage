...
function isPrivateAosMachine(listing = {}) {
  const publicData = publicDataOf(listing);
...
async function loadRealPrivateAosMachine() {
  ...
  for (const authorId of authorIds) {
    try {
      ...
      const candidate = accountListings.find(isPrivateAosMachine);
      if (!candidate) continue;

      return hydrateIXIListingMedia(candidate, {
        dedupeRequests: true
      });
    } catch {
      // Probe the next real production owner. No synthetic fallback is permitted.
    }
  }

  throw new Error(
    "No real private, Passport-backed AOS machine was found in the production owner inventories inspected."
  );
}
...
export default function MobileAosFoundationPage() {
  ...
  useEffect(() => {
    ...
    async function loadMachine() {
      try {
        const realMachine = await loadRealPrivateAosMachine();
        if (cancelled) return;
  ...
  return (
    <>
      <Head>
        <title>IXI Mobile AOS Dollar Launcher Certification</title>
        <meta
          name="viewport"
      ...
      <main className="mobile-aos-shell">
        <header className="certification-header">
          <div>
            ...
            <span>TRANCHE 3 · REAL PRIVATE MACHINE · $ ONLY</span>
          </div>
        ...
        <section className="aos-workspace" aria-busy={status === "loading"}>
          {status === "loading" ? (
            <div className="foundation-message">Loading real AOS machine…</div>
          ) : null}
            ...
            >
              <div
                ...
                style={{
                  ...
                  minHeight: `${metrics.nativeHeight}px`,
