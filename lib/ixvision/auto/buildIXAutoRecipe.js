...
export function buildIXAutoRecipe(scores = {}, options = {}) {
  ...
  const maxWidth = resolutionDamage > 0.55 ? 2600 : 2200;
   ...
   const toolRecipe = {
    exposureRecover: round3(clamp(exposureDamage * 0.32, 0, 0.28)),
    dehaze: round3(clamp(contrastNeed * 0.22, 0, 0.18)),
    deblock: round3(clamp(compressionDamage * 0.75, 0, 0.72)),
    denoise: round3(clamp(compressionDamage * 0.55, 0, 0.55)),
    haloRepair: round3(clamp((sharpnessDamage + compressionDamage) * 0.20, 0, 0.28)),
    upscale: resolutionDamage > 0.55 ? 2 : 1
  };
