...
applyIXExposureRecoverPass(
  ctx,
  width,
  height,
  toolRecipe.exposureRecover || 0
);
...
applyIXDeblockPass(ctx, width, height, {
  amount: toolRecipe.deblock || 0
});

applyIXDenoisePass(ctx, width, height, {
  amount: toolRecipe.denoise || 0
});

applyIXClarityPass(ctx, width, height, protectedPreset);
applyIXRepairPass(ctx, width, height, protectedPreset);

applyIXHaloRepairPass(ctx, width, height, {
  amount: toolRecipe.haloRepair || 0
});
...
// POST FREE needs only the selected treatment; other workbenches still use
// buildIXPhotoVariants when they explicitly need the full editing set.
export async function prepareIXPhoto(file, options = {}) {
  const policy = getUploadPolicy(options);
  const activeMode = normalizeIXPhotoMode(options.mode || "original");
  const processedFile = await processIXPhoto(file, { ...options, mode: activeMode });
  const originalUrl = URL.createObjectURL(file);
  const url = processedFile === file ? originalUrl : URL.createObjectURL(processedFile);
  return {
