...
export function normalizeOwnedMachineListing(listing = {}) {
  ...
  return {
    ...
    // Authenticated server callers supply the persisted listing. Authorship and
    // visibility alone are not evidence that the machine belongs in Equipment.
    ownership: {
      role: clean(publicData.ownershipRole).toLowerCase(),
      status: clean(publicData.ownershipStatus).toLowerCase()
    },
    fields: {
      ...
      location: clean(publicData.location) || [clean(publicData.city), clean(publicData.loc || publicData.state)].filter(Boolean).join(", "),
