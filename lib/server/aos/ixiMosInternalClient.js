import crypto from "crypto";

const DEFAULT_IX_CORE_BASE =
  "https://staging.ironxchange.com/ix-core";

function clean(value) {
  return String(value ?? "").trim();
}

function getOnboardingProfile(session = {}) {
  const currentUser = session?.currentUser || {};
  const attributes = currentUser?.attributes || {};
  const profile = attributes?.profile || currentUser?.profile || {};
  const protectedData = profile?.protectedData || {};
  const personName = clean(
    `${clean(profile?.firstName)} ${clean(profile?.lastName)}`
  );

  return {
    displayName:
      personName ||
      clean(profile?.displayName) ||
      clean(session?.displayName) ||
      "IXI Owner",
    firstName: clean(profile?.firstName),
    lastName: clean(profile?.lastName),
    email: clean(attributes?.email),
    phone: clean(protectedData?.phoneNumber)
  };
}

function getIxCoreBase() {
  return (
    clean(process.env.IX_CORE_BASE_URL) ||
    DEFAULT_IX_CORE_BASE
  ).replace(/\/+$/, "");
}

function getInternalSecret() {
  const secret =
    clean(
      process.env.IXI_MOS_INTERNAL_SECRET
    );

  if (!secret) {
    const error = new Error(
      "IXI MOS internal signing secret is not configured."
    );

    error.code = "IXI_MOS_INTERNAL_SECRET_REQUIRED";
    error.status = 503;
    throw error;
  }

  return secret;
}

function getInternalTimeoutMs() {
  const configured =
    Number(process.env.IXI_MOS_INTERNAL_TIMEOUT_MS);

  if (!Number.isFinite(configured) || configured <= 0) {
    return 20000;
  }

  return Math.min(Math.max(Math.floor(configured), 1000), 60000);
}

function bodyToString(body) {
  if (
    body === undefined ||
    body === null
  ) {
    return "";
  }

  return JSON.stringify(body);
}

function sha256(value) {
  return crypto
    .createHash("sha256")
    .update(value)
    .digest("hex");
}

function createSignature({
  secret,
  timestamp,
  requestId,
  method,
  targetPath,
  principalId,
  entityId,
  bodyString
}) {
  const canonical = [
    timestamp,
    requestId,
    method.toUpperCase(),
    targetPath,
    principalId,
    entityId,
    sha256(bodyString)
  ].join("\n");

  return crypto
    .createHmac("sha256", secret)
    .update(canonical)
    .digest("hex");
}

async function readPayload(response) {
  const text = await response.text();

  if (!text) {
    return null;
  }

  try {
    return JSON.parse(text);
  } catch {
    return {
      raw: text
    };
  }
}

export async function requestIxCoreMos({
  path,
  method = "GET",
  body = null,
  principalId = "",
  entityId = "",
  extraHeaders = {}
} = {}) {
  const normalizedPath =
    clean(path).startsWith("/")
      ? clean(path)
      : `/${clean(path)}`;

  if (!normalizedPath) {
    const error = new Error(
      "IX-Core MOS path is required."
    );

    error.code = "IXI_MOS_INTERNAL_PATH_REQUIRED";
    error.status = 500;
    throw error;
  }

  const secret =
    getInternalSecret();

  const normalizedMethod =
    clean(method).toUpperCase() || "GET";

  const normalizedPrincipalId =
    clean(principalId);

  const normalizedEntityId =
    clean(entityId);

  const bodyString =
    bodyToString(body);

  const timestamp =
    String(Date.now());

  const requestId =
    crypto.randomUUID();

  const targetPath =
    `/mos/v1${normalizedPath}`;

  const signature =
    createSignature({
      secret,
      timestamp,
      requestId,
      method: normalizedMethod,
      targetPath,
      principalId:
        normalizedPrincipalId,
      entityId:
        normalizedEntityId,
      bodyString
    });

  const headers = {
    Accept: "application/json",
    "X-IXI-Internal-Signature-Version": "v1",
    "X-IXI-Internal-Timestamp": timestamp,
    "X-IXI-Internal-Request-Id": requestId,
    "X-IXI-Internal-Principal-Id":
      normalizedPrincipalId,
    "X-IXI-Internal-Entity-Id":
      normalizedEntityId,
    "X-IXI-Internal-Signature": signature,
    ...extraHeaders
  };

  if (bodyString) {
    headers["Content-Type"] =
      "application/json";
  }

  let response;
  const timeoutMs = getInternalTimeoutMs();
  const requestStarted = Date.now();

  try {
    response = await fetch(
      `${getIxCoreBase()}${targetPath}`,
      {
        method: normalizedMethod,
        headers,
        body:
          bodyString || undefined,
        signal: AbortSignal.timeout(timeoutMs)
      }
    );
  } catch (cause) {
    const timedOut =
      cause?.name === "TimeoutError" ||
      cause?.name === "AbortError";

    const error = new Error(
      timedOut
        ? `IX-Core did not respond within ${timeoutMs} ms.`
        : "IXI server gateway could not reach IX-Core."
    );

    error.code = timedOut
      ? "IXI_MOS_INTERNAL_TIMEOUT"
      : "IXI_MOS_INTERNAL_NETWORK_ERROR";
    error.status = timedOut ? 504 : 502;
    error.cause = cause;
    throw error;
  }

  const headersReceived = Date.now();
  const payload =
    await readPayload(response);
  const readOperation = normalizedPath === "/aos/work-bootstrap" ? "bootstrap"
    : normalizedPath === "/aos/context" ? "context"
    : /^\/entities\/[^/]+$/.test(normalizedPath) ? "entity" : null;
  if (readOperation && normalizedMethod === "GET") {
    // Numeric server phases distinguish processing from transport/connection
    // latency without logging identities, financial data or signing headers.
    const serverTiming = response.headers?.get?.("server-timing") || "";
    const core = Object.fromEntries([...serverTiming.matchAll(/ixi-mos-([a-z]+);dur=(\d+(?:\.\d+)?)/g)]
      .map(([, name, ms]) => [name, Number(ms)]));
    console.info("IXI MOS READ TIMING", { operation: readOperation, status: response.status,
      headersMs: headersReceived - requestStarted, bodyMs: Date.now() - headersReceived, core });
  }

  if (
    !response.ok ||
    payload?.ok === false
  ) {
    const error = new Error(
      payload?.error?.message ||
      `IX-Core MOS request failed with status ${response.status}.`
    );

    error.code =
      payload?.error?.code ||
      "IXI_MOS_INTERNAL_REQUEST_FAILED";

    error.status =
      response.status;

    error.details =
      payload?.error?.details ||
      null;

    throw error;
  }

  return payload;
}

export async function requestIxCoreFinancial({
  path,
  method = "GET",
  body = null,
  principalId = "",
  entityId = "",
  extraHeaders = {}
} = {}) {
  const financialPath = clean(path);

  if (!financialPath.startsWith("/financial/")) {
    const error = new Error(
      "IX-Core Financial path must begin with /financial/."
    );
    error.code = "IXI_FINANCIAL_INTERNAL_PATH_INVALID";
    error.status = 500;
    throw error;
  }

  const secret = getInternalSecret();
  const normalizedMethod = clean(method).toUpperCase() || "GET";
  const normalizedPrincipalId = clean(principalId);
  const normalizedEntityId = clean(entityId);
  const bodyString = bodyToString(body);
  const timestamp = String(Date.now());
  const requestId = crypto.randomUUID();

  if (!normalizedPrincipalId || !normalizedEntityId) {
    const error = new Error(
      "Authenticated principal and Entity are required for Financial requests."
    );
    error.code = "IXI_FINANCIAL_INTERNAL_CONTEXT_REQUIRED";
    error.status = 401;
    throw error;
  }

  const signature = createSignature({
    secret,
    timestamp,
    requestId,
    method: normalizedMethod,
    targetPath: financialPath,
    principalId: normalizedPrincipalId,
    entityId: normalizedEntityId,
    bodyString
  });

  let response;

  const financialRequestStarted = Date.now();
  try {
    response = await fetch(`${getIxCoreBase()}${financialPath}`, {
      method: normalizedMethod,
      headers: {
        Accept: "application/json",
        "X-IXI-Internal-Signature-Version": "v1",
        "X-IXI-Internal-Timestamp": timestamp,
        "X-IXI-Internal-Request-Id": requestId,
        "X-IXI-Internal-Principal-Id": normalizedPrincipalId,
        "X-IXI-Internal-Entity-Id": normalizedEntityId,
        "X-IXI-Internal-Signature": signature,
        "X-IXI-Source": "ironxchange-transact",
        ...extraHeaders,
        ...(bodyString ? { "Content-Type": "application/json" } : {})
      },
      body: bodyString || undefined
    });
  } catch (cause) {
    const error = new Error(
      "IXI server gateway could not reach IX-Core Financial."
    );
    error.code = "IXI_FINANCIAL_INTERNAL_NETWORK_ERROR";
    error.status = 502;
    error.cause = cause;
    throw error;
  }

  const financialHeadersReceived = Date.now();
  const payload = await readPayload(response);
  if (financialPath === "/financial/access-context" && normalizedMethod === "GET") {
    const serverTiming = response.headers?.get?.("server-timing") || "";
    const core = Object.fromEntries([...serverTiming.matchAll(/ixi-financial-([a-z]+);dur=(\d+(?:\.\d+)?)/g)]
      .map(([, name, ms]) => [name, Number(ms)]));
    console.info("IXI FINANCIAL ACCESS TIMING", { status: response.status,
      headersMs: financialHeadersReceived - financialRequestStarted,
      bodyMs: Date.now() - financialHeadersReceived, core });
  }

  if (!response.ok || payload?.ok === false) {
    const problem = payload?.errors?.[0] || payload?.error || {};
    const error = new Error(
      (typeof problem === "string" ? problem : problem.message) ||
        `IX-Core Financial request failed with status ${response.status}.`
    );
    error.code =
      (typeof problem === "object" && problem?.code) ||
      "IXI_FINANCIAL_INTERNAL_REQUEST_FAILED";
    error.status = response.status;
    error.details =
      (typeof problem === "object" && problem?.details) || null;
    error.payload = payload;
    throw error;
  }

  return payload;
}

export async function requestIxCoreFreight({
  path,
  method = "GET",
  body = null,
  principalId = "",
  entityId = "",
  extraHeaders = {}
} = {}) {
  const suffix = clean(path).replace(/^\/+/, "");
  const targetPath = `/freight/v1${suffix ? `/${suffix}` : ""}`;
  const secret = getInternalSecret();
  const normalizedMethod = clean(method).toUpperCase() || "GET";
  const normalizedPrincipalId = clean(principalId);
  const normalizedEntityId = clean(entityId);
  const bodyString = bodyToString(body);
  const timestamp = String(Date.now());
  const requestId = crypto.randomUUID();

  if (!normalizedPrincipalId || !normalizedEntityId) {
    const error = new Error("Authenticated principal and Entity are required for Freight requests.");
    error.code = "IXI_FREIGHT_INTERNAL_CONTEXT_REQUIRED";
    error.status = 401;
    throw error;
  }

  const signature = createSignature({
    secret,
    timestamp,
    requestId,
    method: normalizedMethod,
    targetPath,
    principalId: normalizedPrincipalId,
    entityId: normalizedEntityId,
    bodyString
  });

  let response;
  try {
    response = await fetch(`${getIxCoreBase()}${targetPath}`, {
      method: normalizedMethod,
      headers: {
        Accept: "application/json",
        "X-IXI-Internal-Signature-Version": "v1",
        "X-IXI-Internal-Timestamp": timestamp,
        "X-IXI-Internal-Request-Id": requestId,
        "X-IXI-Internal-Principal-Id": normalizedPrincipalId,
        "X-IXI-Internal-Entity-Id": normalizedEntityId,
        "X-IXI-Internal-Signature": signature,
        "X-IXI-Source": "ironxchange-transact-freight",
        ...extraHeaders,
        ...(bodyString ? { "Content-Type": "application/json" } : {})
      },
      body: bodyString || undefined
    });
  } catch (cause) {
    const error = new Error("IXI server gateway could not reach IX-Core Freight.");
    error.code = "IXI_FREIGHT_INTERNAL_NETWORK_ERROR";
    error.status = 502;
    error.cause = cause;
    throw error;
  }

  const payload = await readPayload(response);
  if (!response.ok || payload?.ok === false) {
    const problem = payload?.errors?.[0] || payload?.error || {};
    const error = new Error(problem.message || `IX-Core Freight request failed with status ${response.status}.`);
    error.code = problem.code || "IXI_FREIGHT_INTERNAL_REQUEST_FAILED";
    error.status = response.status;
    error.details = problem.details || null;
    error.payload = payload;
    throw error;
  }

  return payload;
}

export async function requestIxCoreTicket({
  path,
  method = "GET",
  body = null,
  principalId = "",
  entityId = "",
  extraHeaders = {}
} = {}) {
  const suffix = clean(path);
  const targetPath = `/tickets/v1${suffix.startsWith("/") ? suffix : `/${suffix}`}`;
  const secret = getInternalSecret();
  const normalizedMethod = clean(method).toUpperCase() || "GET";
  const normalizedPrincipalId = clean(principalId);
  const normalizedEntityId = clean(entityId);
  const bodyString = bodyToString(body);
  const timestamp = String(Date.now());
  const requestId = crypto.randomUUID();

  if (!normalizedPrincipalId || !normalizedEntityId) {
    const error = new Error(
      "Authenticated principal and Entity are required for Ticket requests."
    );
    error.code = "IXI_TICKET_INTERNAL_CONTEXT_REQUIRED";
    error.status = 401;
    throw error;
  }

  const signature = createSignature({
    secret,
    timestamp,
    requestId,
    method: normalizedMethod,
    targetPath,
    principalId: normalizedPrincipalId,
    entityId: normalizedEntityId,
    bodyString
  });

  let response;
  const timeoutMs = getInternalTimeoutMs();

  try {
    response = await fetch(`${getIxCoreBase()}${targetPath}`, {
      method: normalizedMethod,
      headers: {
        Accept: "application/json",
        "X-IXI-Internal-Signature-Version": "v1",
        "X-IXI-Internal-Timestamp": timestamp,
        "X-IXI-Internal-Request-Id": requestId,
        "X-IXI-Internal-Principal-Id": normalizedPrincipalId,
        "X-IXI-Internal-Entity-Id": normalizedEntityId,
        "X-IXI-Internal-Signature": signature,
        "X-IXI-Source": "ironxchange-ticket-gateway",
        ...extraHeaders,
        ...(bodyString ? { "Content-Type": "application/json" } : {})
      },
      body: bodyString || undefined,
      signal: AbortSignal.timeout(timeoutMs)
    });
  } catch (cause) {
    const timedOut =
      cause?.name === "TimeoutError" ||
      cause?.name === "AbortError";
    const error = new Error(
      timedOut
        ? `IX-Core Ticket service did not respond within ${timeoutMs} ms.`
        : "IXI server gateway could not reach IX-Core Ticket service."
    );
    error.code = timedOut
      ? "IXI_TICKET_INTERNAL_TIMEOUT"
      : "IXI_TICKET_INTERNAL_NETWORK_ERROR";
    error.status = timedOut ? 504 : 502;
    error.cause = cause;
    throw error;
  }

  const payload = await readPayload(response);
  if (!response.ok || payload?.ok === false) {
    const problem = payload?.errors?.[0] || payload?.error || {};
    const error = new Error(
      (typeof problem === "string" ? problem : problem.message) ||
        `IX-Core Ticket request failed with status ${response.status}.`
    );
    error.code =
      (typeof problem === "object" && problem?.code) ||
      "IXI_TICKET_INTERNAL_REQUEST_FAILED";
    error.status = response.status;
    error.details =
      (typeof problem === "object" && problem?.details) || null;
    error.payload = payload;
    throw error;
  }

  return payload;
}

export async function resolveIxCoreAosContext({
  session
}) {
  const userId =
    clean(session?.userId);

  if (!userId) {
    const error = new Error(
      "Authenticated AOS user identity is required."
    );

    error.code = "AOS_BROWSER_USER_ID_REQUIRED";
    error.status = 401;
    throw error;
  }

  const response =
    await requestIxCoreMos({
      path: "/aos/onboarding/bootstrap",
      method: "POST",
      principalId: userId,
      body: {
        ownerUserId: userId,
        entityDisplayName:
          clean(session?.displayName) ||
          "IXI Entity",
        person: getOnboardingProfile(session),
        metadata: {
          source:
            "authenticated-browser-gateway",
          authenticatedThrough:
            "sharetribe"
        }
      }
    });

  const environment =
    response?.environment || null;

  const entityId =
    clean(
      environment?.entity?.entityId
    );

  if (!entityId) {
    const error = new Error(
      "IX-Core did not resolve an AOS Entity for the authenticated user."
    );

    error.code = "AOS_BROWSER_ENTITY_RESOLUTION_FAILED";
    error.status = 502;
    throw error;
  }

  return {
    userId,
    entityId,
    account:
      environment?.account || null,
    principal:
      environment?.principal || null,
    entity:
      environment?.entity || null,
    onboarding:
      response?.onboarding || null,
    environment,
    response
  };
}

export async function resolveExistingIxCoreAosContext({
  session
}) {
  const userId =
    clean(session?.userId);

  if (!userId) {
    const error = new Error(
      "Authenticated AOS user identity is required."
    );

    error.code = "AOS_BROWSER_USER_ID_REQUIRED";
    error.status = 401;
    throw error;
  }

  const response =
    await requestIxCoreMos({
      path: "/aos/context",
      method: "GET",
      principalId: userId
    });

  const context =
    response?.context || null;

  const entityId =
    clean(context?.entityId);

  if (!entityId) {
    const error = new Error(
      "IX-Core did not resolve an existing AOS Entity for the authenticated user."
    );

    error.code = "AOS_BROWSER_ENTITY_RESOLUTION_FAILED";
    error.status = 502;
    throw error;
  }

  return {
    userId,
    entityId,
    accountId: clean(context?.accountId),
    tenantId: clean(context?.tenantId),
    membershipId: clean(context?.membershipId),
    response
  };
}

export async function resolveExistingIxCoreAosEnvironment({
  session
}) {
  const userId = clean(session?.userId);
  if (!userId) {
    const error = new Error("Authenticated AOS user identity is required.");
    error.code = "AOS_BROWSER_USER_ID_REQUIRED";
    error.status = 401;
    throw error;
  }

  let response;
  try {
    response = await requestIxCoreMos({
      path: "/aos/work-bootstrap",
      method: "GET",
      principalId: userId
    });
  } catch (error) {
    /* Only a genuinely new user enters the governed creation boundary. */
    if (Number(error?.status) === 404) {
      return resolveIxCoreAosContext({ session });
    }

    throw error;
  }

  const environment = response?.environment || null;
  const entityId = clean(environment?.entity?.entityId);

  if (!entityId) {
    const error = new Error(
      "IX-Core returned no governed Entity in the AOS Work bootstrap."
    );
    error.code = "AOS_BROWSER_ENTITY_RESOLUTION_FAILED";
    error.status = 502;
    throw error;
  }

  return {
    userId,
    entityId,
    accountId: clean(environment?.account?.accountId),
    tenantId: clean(environment?.account?.tenantId),
    account: environment?.account || null,
    principal: environment?.principal || null,
    entity: environment?.entity || null,
    onboarding: null,
    environment,
    response,
    resolutionMode: "single-work-bootstrap"
  };
}

// TRAN$ACT needs a verified principal and Entity, not a rebuilt AOS estate.
// Never cache authority or turn an authorization failure into onboarding.
export async function resolveIxCoreTransactContext({
  session,
  includeEntity = false,
  allowOnboarding = false
}) {
  let context;
  try {
    context = await resolveExistingIxCoreAosContext({ session });
  } catch (error) {
    if (allowOnboarding && Number(error?.status) === 404 &&
        error?.code === "AOS_ACCOUNT_NOT_FOUND") {
      return resolveIxCoreAosContext({ session });
    }
    throw error;
  }

  if (!includeEntity) return context;
  const result = await requestIxCoreMos({
    path: `/entities/${encodeURIComponent(context.entityId)}`,
    principalId: context.userId,
    entityId: context.entityId
  });
  if (!result?.entity || clean(result.entity.entityId) !== context.entityId) {
    const error = new Error("IX-Core returned a different or missing operating Entity.");
    error.code = "IXI_TRANSACT_ENTITY_CONTEXT_INVALID";
    error.status = 502;
    throw error;
  }
  return { ...context, entity: result.entity };
}

export default {
  requestIxCoreMos,
  requestIxCoreFinancial,
  requestIxCoreFreight,
  requestIxCoreTicket,
  resolveIxCoreAosContext,
  resolveExistingIxCoreAosEnvironment,
  resolveExistingIxCoreAosContext,
  resolveIxCoreTransactContext,
  getOnboardingProfile
};
