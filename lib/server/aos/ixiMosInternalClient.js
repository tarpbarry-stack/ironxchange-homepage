...
export async function resolveExistingIxCoreAosEnvironment({
  ...
  const response = await requestIxCoreMos({
    path: "/aos/environment",
    method: "POST",
    principalId: context.userId,
    entityId: context.entityId,
    body: {
      displayName: clean(session?.displayName) || "IXI Entity",
      metadata: {
        source: "authenticated-browser-existing-environment",
  ...
  if (!entityId || entityId !== context.entityId) {
    const error = new Error(
      "IX-Core returned an AOS environment outside the authenticated Entity context."
    );
  ...
  return {
    ...
    environment,
    response,
    resolutionMode: "existing-environment"
  };
}
