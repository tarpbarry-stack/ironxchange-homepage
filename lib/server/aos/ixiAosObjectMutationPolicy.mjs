...
export function assertAosObjectMutationRequest({
  ...
  const ifMatchRevision = parseRevision(headers["if-match"], "If-Match");
  if (ifMatchRevision !== expectedRevision) {
    fail(
      "AOS_OBJECT_REVISION_MISMATCH",
      "If-Match and expectedRevision must identify the same object revision.",
      412,
      { expectedRevision, ifMatchRevision }
    );
  }
