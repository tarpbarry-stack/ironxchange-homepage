...
export async function deleteRemoteTicket(ticketId, expectedRevision, reason = "obsolete") {
  if (!ticketId) throw new Error("Ticket ID is required for deletion.");
  if (!Number.isInteger(expectedRevision) || expectedRevision < 1) {
    throw new Error("A synchronized Ticket revision is required for deletion.");
  }
...
export async function startRemoteTicket(ticket, options = {}) {
  ...
  const leaseExpiry = Date.parse(remote.metadata?.execution?.leaseExpiresAt || "");
  const recovery = remote.status === "working" && (!Number.isFinite(leaseExpiry) || leaseExpiry <= Date.now());
  if (!["ready-for-chat", "reopened"].includes(remote.status) && !recovery) return remote;
