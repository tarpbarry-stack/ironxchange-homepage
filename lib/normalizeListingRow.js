...
function isValidMake(value) {
  const make = String(value || "").trim().toUpperCase();

  return VALID_MAKES.includes(make);
}
...
export function normalizeListingRow(row, index = 0) {
  ...
  } else if (!isValidMake(normalized.make)) {
    errors.push(`Invalid make: ${normalized.make}`);
  }

  if (!normalized.model) {
    errors.push("Missing model");
  } else if (isMostlyNumeric(normalized.model)) {
    errors.push(`Model looks wrong: ${normalized.model}`);
  }
  ...
  if (!normalized.location) errors.push("Missing location");
