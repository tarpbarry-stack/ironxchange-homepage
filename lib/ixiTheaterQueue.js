...
export async function saveTheaterQueue({
  ...
  if (!userId) {
    throw new Error(
      "saveTheaterQueue requires a resolved IXI user identity."
    );
  }
...
export async function sendMachinesToTheater({
  ...
  if (!userId) {
    throw new Error(
      "sendMachinesToTheater requires a resolved IXI user identity."
    );
  }
  ...
  const remoteIxiResponse = await fetchIxiMachineState(userId);
const remoteState = remoteIxiResponse?.state || remoteIxiResponse || {};
const savedQueue = remoteState?.[IXI_THEATER_QUEUE_ID];
  ...
  console.log("THEATER SEND LIB WRITE", {
  userId,
  cleanIds,
  receptor,
  targetReceptor,
...
export async function sendMachineToTheater({
  ...
  if (!userId) {
    throw new Error(
      "sendMachineToTheater requires a resolved IXI user identity."
    );
  }
