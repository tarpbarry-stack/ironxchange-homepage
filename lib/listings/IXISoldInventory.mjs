...
export function summarizeSoldListings(listings = []) {
  const totals = new Map();
  let missingPriceCount = 0, returnedCount = 0;
  for (const { soldSummary: sale } of listings) {
    if (sale.status === "returned") { returnedCount++; continue; }
    const amount = sale.salePrice;
    if (typeof amount !== "number" || !Number.isFinite(amount)) { missingPriceCount++; continue; }
    const currency = clean(sale.currency).toUpperCase() || "USD";
    totals.set(currency, (totals.get(currency) || 0) + Math.round(amount * 100));
  }
  return { totals: [...totals].sort(([a], [b]) => a.localeCompare(b)).map(([currency, amountCents]) => ({ currency, amountCents })), missingPriceCount, returnedCount };
}
export function querySoldListings(listings, query = {}) {
  ...
  return { listings: filtered.slice((page - 1) * pageSize, page * pageSize), total, page, pageSize, salesSummary: summarizeSoldListings(filtered),
