...
export function getListingHref(item = {}, from = "browse") {
  ...
  const descriptor =
    slugify(
      [
        item?.year || publicData?.year,
        item?.make || publicData?.make,
        item?.model || publicData?.model
      ]
        .filter(Boolean)
        .join(" ") ||
        cleanMachineTitle(
  ...
  const basePath =
    passportId
      ? `/listing/${descriptor}--${passportId}`
      : `/listing/${slugify(
          item?.title || "listing"
        )}`;

  return `${basePath}?from=${encodeURIComponent(
    from
  )}`;
}
...
export function getCardImages(item = {}) {
  ...
  const imageObjects = Array.isArray(item.imageObjects)
    ? item.imageObjects
    : [];
