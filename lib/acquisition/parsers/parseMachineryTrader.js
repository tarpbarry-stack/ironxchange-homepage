...
function getLocation(product, specs) {
  ...
  const match = raw.match(/([A-Za-z .'-]+),?\s+(Texas|Oklahoma|Louisiana|Arkansas|New Mexico|Kansas|Missouri|Colorado|California|Florida|Georgia|Alabama|Mississippi|Tennessee|Kentucky|Ohio|Michigan|Indiana|Illinois|Iowa|Nebraska|South Dakota|North Dakota|Minnesota|Wisconsin|Pennsylvania|New York|North Carolina|South Carolina|Virginia|West Virginia|Arizona|Nevada|Utah|Idaho|Montana|Wyoming|Oregon|Washington)\s*(\d{5})?/i);
...
export async function parseMachineryTrader(url = "") {
