...
function extractImages(html) {
  ...
  const productSection =
    html.match(/<div class=["'][^"']*product_imgs_wrap[^"']*["'][\s\S]*?<div class=["'][^"']*product_info_right/i)?.[0] ||
    html;

  const bgMatches = productSection.matchAll(
    /background:\s*url\((https:\/\/www\.4saleheavyequipment\.com\/hubfs\/repo\/[^)]+)\)/gi
  );

  for (const match of bgMatches) {
    const url = clean(match[1]);

    if (url && !url.includes("cloned")) {
      urls.add(url);
    }
  }

  const imgMatches = productSection.matchAll(
    /<img[^>]+src=["']([^"']+)["'][^>]*>/gi
  );
...
function extract4SalePrice(html) {
  return matchFirst(html, [
    /<div[^>]+class=["'][^"']*\brate\b[^"']*["'][^>]*>\s*\$?\s*([\d,.]+)/i
  ]);
}
...
export async function parse4SaleHeavyEquipment(url = "") {
  ...
  const price =
  facts.price ||
  extract4SalePrice(html) ||
  extractLabeledValue(html, ["Price", "Sale Price", "Asking Price"]);

const location = extractLabeledValue(html, ["Location"]);
const [city, state] = location.split(",").map(clean);

const serialNumber = extractLabeledValue(html, [
  "Serial",
  "Serial Number",
  "Serial #",
  "S/N"
...
const stockNumber = extractLabeledValue(html, [
  "ID",
  "Stock",
  "Stock Number",
  "Inventory ID",
