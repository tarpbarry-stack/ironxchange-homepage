...
function extractPhotos(html, title) {
  ...
  const escapedCdnMatches = html.matchAll(
    /"uri"\s*:\s*"((?:https:\\\/\\\/|https:\/\/)[^"]+\.(?:jpg|jpeg|png|webp)[^"]*)"/gi
  );

  for (const match of escapedCdnMatches) {
  ...
  const productPhotoMatches = html.matchAll(
    new RegExp(
      `<img[^>]+alt=["'][^"']*${title.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}[^"']*["'][^>]+src=["']([^"']+)["']`,
      "gi"
    )
  );

  for (const match of productPhotoMatches) {
    const src = decodeHtml(match[1]);

    // Browser-saved files use local ./_files paths. Those prove image count,
    // but they are not remote-publishable URLs, so only keep real URLs here.
    if (/^https?:\/\//i.test(src)) {
      urls.add(src);
    }
  }
