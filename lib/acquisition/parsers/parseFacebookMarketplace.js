...
function extractPhotos(html = "") {
  ...
  const listingStart = normalized.indexOf('"listing_photos"');
  const listingStart = normalized.indexOf('"listing_photos"');

let source = normalized;

if (listingStart >= 0) {
  const afterPhotos = normalized.slice(listingStart);
  ...
  const stopMarkers = [
    ...
    '"CometMarketplacePDPRelatedItemsQuery"'
  ];

  const stopIndex = stopMarkers
    .map(marker => afterPhotos.indexOf(marker))
    .filter(index => index > 0)
    .sort((a, b) => a - b)[0];

  source =
    stopIndex > 0
      ? afterPhotos.slice(0, stopIndex)
      : afterPhotos.slice(0, 30000);
}
  ...
  console.log("FB FINAL PHOTO COUNT:", urls.size);
