...
function extractPhotos(html = "") {
  ...
  const listingStart = decoded.indexOf('"listing_photos"');

  if (listingStart >= 0) {
    const listingSection = decoded.slice(listingStart);

    const matches = listingSection.matchAll(
    ...
    for (const match of matches) {
      ...
      if (isFacebookMachinePhoto(url)) {
        console.log("FB EXTRACTED PHOTO URL:", url);
        urls.add(url);
      }
    }
  }
