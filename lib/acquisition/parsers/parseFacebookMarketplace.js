...
function extractPhotos(html = "", listingId = "") {
  ...
  for (const script of scripts) {
    ...
    if (
      !raw.includes("MarketplacePDPC2CMediaViewerWithImagesQuery") &&
      !raw.includes("listing_photos")
    ) {
      continue;
    }
    ...
    try {
      ...
      // Ignore malformed Facebook payload chunks.
    }
  }
  ...
  if (urls.size === 0) {
    ...
    if (markerIndex >= 0) {
      const section = normalized.slice(markerIndex, markerIndex + 30000);

      const matches = section.matchAll(
        /"image"\s*:\s*\{[\s\S]*?"uri"\s*:\s*"([^"]+)"/gi
      );
