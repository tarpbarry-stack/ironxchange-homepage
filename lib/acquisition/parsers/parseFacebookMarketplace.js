...
function extractPhotos(html = "") {
  ...
  // 1. Best source: Facebook's actual PDP media viewer preloads
  const preloadMatches = normalized.matchAll(
    /<link[^>]+href=["']([^"']+)["'][^>]+as=["']image["'][^>]+data-preloader=["'][^"']*MarketplacePDPC2CMediaViewerWithImagesQuery[^"']*["'][^>]*>/gi
  );
  ...
  // 2. Structured fallback: exact listing_photos array
  if (urls.size <= 1) {
    const markerIndex = normalized.indexOf('"listing_photos"');
  ...
  console.log("FB PHOTO COUNT:", urls.size);
...
export async function parseFacebookMarketplace(url = "", htmlOverride = "") {
  ...
  console.log("FB HTML LENGTH:", html.length);  const media = extractPhotos(html);
