...
function extractPhotos(html = "", listingId = "") {
  ...
  for (const script of scripts) {
    ...
    if (!raw.includes("MarketplacePDPC2CMediaViewerWithImagesQuery")) continue;
    ...
    try {
      ...
      console.log("FB PHOTO JSON PARSE FAILED:", error.message);
    }
  }
...
async function fetchFacebookHtml(url) {
  const fetchUrl = String(url).replace(
...
export async function parseFacebookMarketplace(url = "", htmlOverride = "") {
  ...
  console.log("FB HAS PRICE TEXT:", html.includes("$78,000"));
console.log("FB HAS BERKELEY:", html.includes("Berkeley"));
console.log("FB HAS VOLVO:", html.includes("Volvo"));
console.log("FB HAS SD115B:", html.includes("SD115B"));
console.log("FB HAS HOURS 1142:", html.includes("1,142"));
console.log("FB HAS DESCRIPTION BODY:", html.includes("For sale: 2019 Volvo"));
