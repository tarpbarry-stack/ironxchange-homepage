...
function extractPhotos(html = "") {
  ...
  // Primary: Facebook Marketplace structured gallery payload
  const listingPhotoBlocks = decoded.matchAll(
    /"listing_photos"\s*:\s*\[([\s\S]*?)\]\s*,\s*"__isMarketplaceListingRenderable"/gi
  );

  for (const block of listingPhotoBlocks) {
    const photoBlock = block[1] || "";

    const uriMatches = photoBlock.matchAll(
      /"uri"\s*:\s*"([^"]+)"/gi
    );
    ...
    for (const match of uriMatches) {
      ...
      if (isFacebookMachinePhoto(url)) {
        urls.add(url);
      }
    }
  }

  // Fallback: any product-gallery image URI in saved/fetched HTML
  if (urls.size === 0) {
    const uriMatches = decoded.matchAll(
      /"image"\s*:\s*\{[^{}]*"height"\s*:\s*\d+[^{}]*"width"\s*:\s*\d+[^{}]*"uri"\s*:\s*"([^"]+)"/gi
    );
  ...
  // Final fallback: og:image gives at least hero photo
  if (urls.size === 0) {
    const ogImage = matchFirst(decoded, [
...
export async function parseFacebookMarketplace(url = "", htmlOverride = "") {
  ...
  const media = extractPhotos(html);
