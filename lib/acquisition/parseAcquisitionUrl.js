...
} from "./parsers/parseFacebookMarketplace";
...
export async function parseAcquisitionUrl(url = "") {
  ...
  if (source.type === "facebook-marketplace") {
  return parseFacebookMarketplace(url);
}

  if (source.type === "sandhills") {
    return parseMachineryTrader(url, source.html);
  }
