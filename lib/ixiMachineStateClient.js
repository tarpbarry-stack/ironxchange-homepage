...
export async function fetchIxiMachineState(userId = "guest", { strict = false } = {}) {
  try {
    const requestVersion = Date.now();

    const res = await fetch(
      `${IX_CORE_BASE}/ixi-machine-state/${encodeURIComponent(userId)}?requestVersion=${requestVersion}`,
      {
        cache: "no-store",
        headers: {
    ...
    if (!res.ok) {
      if (strict) throw new Error(`Machine state unavailable (${res.status})`);
      return {};
    }
