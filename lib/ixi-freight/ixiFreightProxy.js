...
const { createIXIGatewayTiming, withIXIGatewayTimeout } = require("../ixi-financial/ixiGatewayTiming");
...
async function proxyIXIFreightRequest({ req, res, path = "", method = "GET", body, timeoutMs = 20000 }) {
  ...
  const timing = createIXIGatewayTiming(res, "freight");
  try {
    ...
    const context = await timing.measure("context", () => clientModule.resolveIxCoreTransactContext({ session }));
    const trustedBody =
      clean(path).replace(/^\/+/, "") === "orders" && clean(method).toUpperCase() === "POST"
        ? await authorizePassportFirstAsset({ userId: session.userId, body })
        : body;
    const payload = await timing.measure("upstream", () => withIXIGatewayTimeout(
      ...
      { timeoutMs, code: "IXI_FREIGHT_UPSTREAM_TIMEOUT", message: "IXI Freight service timed out." }
    ));
