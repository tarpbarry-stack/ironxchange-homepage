...
export function isExplicitAosSystemIndexObject(object = {}) {
  ...
  // Match IX-Core's persisted structural declarations. Appearance and adapter
  // presentation cannot promote an ordinary Object to a System Index.
  return (
...
export function getAosSystemIndexMembershipPolicy(index = {}) {
  if (!isExplicitAosSystemIndexObject(index)) return null;
  // Only the persisted adapter identity can select a platform policy.
  // Presentation indexId/objectId aliases do not carry that authority.
  const adapter = getIXIAosSystemAdapter(clean(index?.metadata?.adapterId));
  if (adapter && adapter.adapterId === clean(index?.metadata?.adapterId)) {
...
export function evaluateAosSystemIndexMembership({
  ...
  const objectType = cleanKey(sourceObject?.objectType);
  const definitionId = clean(sourceObject?.definitionId);
  ...
  if (objectType === "generic" && !definitionId) {
    return { allowed: false, reason: "member-classification-required", policy };
  }
