...
function authoritativeAdmissionAliases(canonicalObject = {}, identityAliases = []) {
  const metadata = safeObject(canonicalObject?.metadata);
  const provisioning = safeObject(metadata?.provisioning || canonicalObject?.provisioning);
  const candidates = [
    ...safeArray(identityAliases),
    ...identitiesOf(canonicalObject),
    canonicalObject?.sourceType || canonicalObject?.sourceId
      ? {
          sourceType: canonicalObject?.sourceType,
          sourceId: canonicalObject?.sourceId
        }
      : null,
    canonicalObject?.sourceListingId
      ? {
          sourceType: "sharetribe-listing",
          sourceId: canonicalObject.sourceListingId
        }
      : null,
    metadata?.sourceListingId
      ? {
          sourceType: "sharetribe-listing",
          sourceId: metadata.sourceListingId
        }
      : null,
    provisioning?.sourceListingId
      ? {
          sourceType: "sharetribe-listing",
          sourceId: provisioning.sourceListingId
        }
      : null
  ].filter(Boolean).map(alias => ({
    sourceType: cleanLower(
      alias?.sourceType ||
      alias?.identityType ||
      alias?.type ||
      alias?.kind
...
export function normalizeIxCoreAdmissionEnvelope({
  ...
  const authoritativeAliases = authoritativeAdmissionAliases(
    canonicalObject,
    identity.aliases
  );
...
function canReadPreviewMedia(item) {
...
function collectAosPreviewImageUrls(object = {}) {
  const candidates = [
    object?.imageUrl,
    ...safeArray(object?.imageUrls),
    object?.primaryImageUrl,
    object?.fields?.primaryImageUrl,
    object?.metadata?.primaryImageUrl,
    object?.presentation?.primaryImageUrl,
    ...safeArray(object?.media)
      .filter(canReadPreviewMedia)
...
export function createAosObjectPreviewReference(objectId, admission) {
  ...
  const imageUrls = collectAosPreviewImageUrls(object);
  return Object.freeze({
...
export function shouldRegisterAosWorkspaceObject({
  admittedObject,
  projectedIndex = null,
  ...
  /*
   * Missing numbered-card presentation quarantines an ordinary object, but
   * must never remove a governed System Index. System Index presentation is
   * supplied by its registered adapter, not by a numbered-card fallback on
   * the backing object.
   */
  return !(
    admittedObject?.presentation?.kind === "unresolved-presentation" &&
    !projectedIndex
  );
}
