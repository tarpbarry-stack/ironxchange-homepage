...
function collectAliases(source = {}) {
  ...
  const metadata = safeObject(source?.metadata);
  const provisioning = safeObject(metadata?.provisioning || source?.provisioning);
  const passportId = getCanonicalAosPassportId(source);
  ...
  identitiesOf(source).forEach(identity => {
...
export function resolveAosCanonicalPresentation({ object = {}, listing = null } = {}) {
  const classification = cleanLower(
    object?.classification?.objectType ||
    object?.objectType ||
    object?.type ||
  ...
  if (classification === "machine" && listing) {
    return Object.freeze({
  ...
  return Object.freeze({
    ...
    templateNumber: explicitCardNumber(object) || 7,
    templateSlug: selectedTemplateSlug(object),
    explicit: Boolean(explicitCardNumber(object) || selectedTemplateSlug(object))
  });
}
...
export function normalizeAosCanonicalObject({ object = {}, listing = null } = {}) {
  ...
  const presentation = resolveAosCanonicalPresentation({ object, listing });
  ...
  return {
    ...
    authorizedPreviews: listing ? Object.freeze([{ source: "sharetribe-listing", alias: listingIdOf(listing) }]) : Object.freeze([]),
