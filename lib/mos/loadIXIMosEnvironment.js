...
function canonicalAdmissionRecord(response = {}) {
...
async function admitCanonicalObjects({
  ...
  for (let offset = 0; offset < objects.length; offset += batchSize) {
    ...
    const results = await Promise.all(batch.map(async object => {
      ...
      const response = await admitMosCanonicalIdentity({
        objectId: requestedObjectId,
        passportId: requestedPassportId,
        aliases: getAdmissionAliases(object, ownedListings)
      });
      const admission = canonicalAdmissionRecord(response);
      const canonicalObject = admission?.object || admission?.canonicalObject;
      const objectId = clean(admission?.objectId || canonicalObject?.objectId);
      const passportId = clean(admission?.passportId || admission?.identity?.passportId);
      const admittedEntityId = clean(
        admission?.entityId ||
        admission?.identity?.entityId ||
        canonicalObject?.entityId
      );
      const verifiedAliases = (Array.isArray(admission?.aliases)
        ? admission.aliases
        : []).map(alias => ({
          ...alias,
          serverVerified: true
      ...
      if (
        ...
        objectId !== requestedObjectId ||
        passportId !== requestedPassportId ||
        admittedEntityId !== clean(entityId)
      ) {
        ...
        error.details = { requestedObjectId, requestedPassportId, objectId, passportId, admittedEntityId };
        throw error;
      }
      ...
      return {
        ...
        objectId,
        passportId,
        entityId: admittedEntityId,
        aliases: verifiedAliases,
        resolutionEvidence: admission?.resolutionEvidence || admission?.evidence || null,
        canonicalAdmissionVerified: true
      };
    }));
    admitted.push(...results);
  }
