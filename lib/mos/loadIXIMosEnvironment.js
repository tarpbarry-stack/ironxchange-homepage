...
async function admitCanonicalObjects({
  ...
  const admitted = [];
  const boundedBatchSize = Math.max(1, Math.min(32, Number(batchSize) || 12));

  for (let offset = 0; offset < objects.length; offset += boundedBatchSize) {
    const batch = objects.slice(offset, offset + boundedBatchSize);
...
export async function loadIXIMosEnvironment({
  ...
  canonicalAdmissionBatchSize = 12
} = {}) {
  ...
  const aosObjects = await admitCanonicalObjects({
    ...
    batchSize: canonicalAdmissionBatchSize
  });
