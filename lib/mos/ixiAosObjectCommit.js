...
export function normalizeExternalCommitEnvelope(
  ...
  return {
    ...
    input: {
      ...
      definitionId:
        clean(object.definitionId) ||
        null,

      definitionKey:
        clean(object.definitionKey) ||
        null,
...
async function updateStudioObject({
  ...
  await updateMosObject({
    ...
    fields:
      safeObject(objectDraft.fields),
    media:
      safeArray(objectDraft.media),
    metadata,
    actorId:
      clean(actorId) || null
  });
...
export async function commitAosStudioLaunch({
  ...
  const mode =
    clean(
      launchPayload?.studio?.mode
    ).toLowerCase();

  const objectId =
    clean(
      launchPayload?.object?.objectId
    );
