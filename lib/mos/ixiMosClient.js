...
export function fetchMosObjectDefinition({
  ...
  return requestMos(
    `/entities/${encodeURIComponent(
      entityId
    )}/object-definitions/${encodeURIComponent(
      definitionId
    )}`,
    {
      signal
...
export function fetchMosCardTemplate({
  ...
  return requestMos(
    `/card-templates/${encodeURIComponent(
      templateSlug
    )}${
      query
        ? `?${query}`
...
export function createMosObject({
  ...
  /* Legacy technical creation path only. */
  objectType = null,
