...
export function getAosCorroboratedLegacyMembershipObjectIds({
  ...
  // Match IX-Core's rail projection: a governed membership supersedes legacy
  // direct-container evidence even when it names a different container. Keep
  // the old records intact; they cannot add a preview absent from BOARD/Recall.
  const governedMemberObjectIds = new Set(
    (Array.isArray(relationships) ? relationships : [])
      .map(relationshipRecord)
      .filter(isAosMembershipRelationship)
      .map(relationship => canonicalReference({
        reference: relationship?.sourceObjectId,
        passportId: relationship?.sourcePassportId || relationship?.sourceIdentity?.passportId,
        admission,
        role: "member"
      }))
  );
