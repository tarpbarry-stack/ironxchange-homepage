...
export function isExplicitAosSystemIndexObject(object = {}) {
  ...
  return (
    ...
    clean(object?.objectType).toLowerCase() === "system-index"
  );
}
...
export function normalizeAosRailProjectionHierarchy({
  ...
  const normalizeMembers = (members, ownerObjectId) =>
    (Array.isArray(members) ? members : []).filter(member => {
      if (isGovernedProjectionMember(member)) return true;
      ...
      return !(
        isExplicitAosSystemIndexObject(objectsById.get(memberObjectId)) &&
        isExplicitAosSystemIndexObject(objectsById.get(ownerObjectId))
      );
    });
...
export async function createAosMembershipRelationship({
  ...
  if (
    isExplicitAosSystemIndexObject(memberObject) &&
    isExplicitAosSystemIndexObject(parentObject)
  ) {
    const error = new Error("System Indexes are peer roots and cannot contain each other.");
    error.code = "IXI_AOS_SYSTEM_INDEX_NESTING_PROHIBITED";
    throw error;
  }
