...
function isExplicitSystemIndexObject(object = {}) {
  const metadata =
    object?.metadata &&
...
export function normalizeAosRailProjectionHierarchy({
  ...
  const normalizeMembers = (members, ownerObjectId) =>
    (Array.isArray(members) ? members : []).filter(member => {
      ...
      return !(
        isExplicitSystemIndexObject(objectsById.get(memberObjectId)) &&
        isExplicitSystemIndexObject(objectsById.get(ownerObjectId))
      );
    });
