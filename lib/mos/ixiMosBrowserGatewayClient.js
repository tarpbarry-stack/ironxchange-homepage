...
export async function createMosRelationship({
  ...
  const readback = await fetchMosObjectRelationships(targetObjectId, {
    direction: "incoming",
    signal
  });
  const verified = (readback?.relationships || []).some(item => {
    const record = item?.relationship || item;
    return record?.relationshipId === relationship.relationshipId &&
