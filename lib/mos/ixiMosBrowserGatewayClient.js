...
export function placeMosObject({
  ...
  const resolvedCommandId =
    clean(commandId) ||
    createMosCommandId("aos-container-place");
...
export async function commitMosContainerPlacement({
  objectId,
  destinationContainerId,
  actorId = null,
  commandId = null,
  ...
  const response = await placeMosObject({
    objectId,
    destinationContainerId,
    actorId,
    commandId: resolvedCommandId,
  ...
  const readback = await fetchMosObject(objectId, { signal });
  const canonicalObject = extractMosObject(readback);

  if (
    !canonicalObject ||
    clean(canonicalObject.directContainerId) !==
      clean(destinationContainerId)
  ) {
    const error = new Error(
      "IX Core canonical readback did not confirm container placement."
    );
    error.code = "IXI_AOS_CONTAINER_READBACK_REQUIRED";
    throw error;
  }
...
export async function createMosRelationship({
  ...
  if (!sourceObjectId || !targetObjectId || !clean(relationshipType)) {
    throw new Error(
      "sourceObjectId, targetObjectId, and relationshipType are required."
    );
  }
  ...
  const relationship = response?.result?.relationship;
  ...
  const verified = (readback?.relationships || []).some(item =>
    item?.relationship?.relationshipId === relationship.relationshipId &&
    item?.relatedObject?.objectId === targetObjectId
  );
