...
export async function uploadPostFreePhotos({
  ...
  const worker = async () => {
    while (!failure && cursor < files.length) {
      ...
      let restartTimer = () => {};
      const progress = () => {
        if (entry.loaded > loaded[index]) restartTimer();
        loaded[index] = entry.loaded;
        report();
      };
        ...
        results[index] = await Promise.race([
          ...
          new Promise((_, reject) => {
            restartTimer = () => {
              clearTimeout(timer);
              timer = setTimeout(() => {
                const error = new Error("Photo upload has stopped reporting progress. Keep this page open and retry to reuse uploaded photos.");
