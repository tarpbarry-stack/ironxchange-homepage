...
export function atlasHelpHref({ pathname = "", moduleId = "", topic = "" } = {}) {
  const resolved = knownAtlasTopic(topic) || atlasTopicForPage(pathname, moduleId);
  const family = { private: "private", auction: "auction", "url-import": "reference" }[resolved];
  return family ? `/atlas?family=${family}` : `/atlas?topic=${resolved}`;
}
