// /lib/passport/ixiPassportSnEngine.js
...
export function generatePassportSerial() {
  let serial = "";
...
export function isValidPassportId(value = "") {
  ...
  if (normalized.length !== PASSPORT_PREFIX.length + PASSPORT_SERIAL_LENGTH) return false;
...
export function getPassportUrl(passportId = "") {
...
export function getMachinePassportId(machine = {}) {
  const publicData = machine?.attributes?.publicData || machine?.publicData || {};

  return normalizePassportId(publicData.passportId || "");
}

export function ensureMachinePassportId(machine = {}) {
  const existing = getMachinePassportId(machine);

  if (isValidPassportId(existing)) {
    return existing;
  }
...
export const IXI_PASSPORT_SN_ENGINE = {
  prefix: PASSPORT_PREFIX,
  alphabet: PASSPORT_ALPHABET,
  serialLength: PASSPORT_SERIAL_LENGTH,
  generatePassportSerial,
  generatePassportId,
  normalizePassportId,
  isValidPassportId,
  getPassportUrl,
  getMachinePassportId,
  ensureMachinePassportId
};
