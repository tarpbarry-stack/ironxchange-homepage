...
export function saveUserIxiPreference({
  userId,
  key,
  value
}) {
  ...
  store.users[uid].__preferences =
    store.users[uid].__preferences || {};

  store.users[uid].__preferences[key] = value;

  saveIxiMachineStateStore(store);

  return store.users[uid].__preferences;
}

export function getUserIxiPreferences(userId) {
  const store = loadIxiMachineStateStore();

  return (
    store.users?.[String(userId || "guest")]?.__preferences ||
    {}
  );
}
