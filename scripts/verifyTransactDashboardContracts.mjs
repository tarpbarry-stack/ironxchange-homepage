...
const noStatement = adapter.createIXIReconciliationDesktopModel({treasury:{accounts:[{accountId:"CASH-1",name:"Operating",bookBalance:1000,lastStatementBalance:null}]}});
...
const outOfBalance = adapter.createIXIReconciliationDesktopModel({treasury:{accounts:[{accountId:"CASH-2",name:"Operating",bookBalance:1000,lastStatementBalance:975}]}});
...
const balanced = adapter.createIXIReconciliationDesktopModel({treasury:{accounts:[{accountId:"CASH-3",name:"Operating",bookBalance:1000,lastStatementBalance:1000}]}});
...
assert.deepEqual(queryContract.getIXITransactWorkspaceIncludes("sales"), ["ar", "reporting"], "Asset Sales must have canonical asset-sale invoice truth plus reporting economics.");
assert.deepEqual(queryContract.getIXITransactWorkspaceIncludes("invoices"), ["ar"]);
...
const valid = queryContract.validateIXITransactDashboardQuery(queryContract.createIXITransactDashboardQuery({scope:{entityPassportIds:["passport:entity:test"]},period:{accountingPeriod:"2026-08"},include:["ar"]}));
