...
export default function ConnectedHomepage() {
  ...
  return <div className={styles.page}>
    ...
    <header className={styles.siteHeader}><Link href="/gateway" className={styles.brand} aria-label="IronXchange gateway"><Image src="/images/ironxchange-logo.png" width={1807} height={396} priority alt="IronXchange" /></Link><nav aria-label="IronXchange products">{NAV.map(([href, label]) => <Link href={href} key={label}>{label}</Link>)}</nav><div className={styles.headerActions}><AtlasHelpLink topic="overview" /><Link href="/browse-v2"><Icon name="search" /><span>SEARCH</span></Link><Link href="/login">SIGN IN</Link><Link href="/post-free" className={styles.postMachine}>POST A MACHINE</Link></div></header>
    <main>
