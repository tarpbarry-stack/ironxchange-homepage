...
function normalizeMachine(listing) {
  ...
  return {
    ...
    year: clean(listing.year || data.year, FALLBACK_MACHINE.year),
    make: clean(listing.make || data.make, FALLBACK_MACHINE.make).toUpperCase(),
    model: clean(listing.model || data.model, FALLBACK_MACHINE.model).toUpperCase(),
    hours: clean(listing.hours || data.hours, FALLBACK_MACHINE.hours),
    price: clean(listing.price, FALLBACK_MACHINE.price),
    location: clean(listing.location || data.loc, FALLBACK_MACHINE.location).toUpperCase(),
    passportId: clean(listing.passportId || data.passportId, FALLBACK_MACHINE.passportId),
  };
}
...
function PassportCard({ machine, compact = false }) {
  return <article className={`${styles.passportCard} ${compact ? styles.passportCompact : ""}`}>
    <header><Icon name="passport" /><span><small>IXI PASSPORT</small><b>{machine.passportId}</b></span></header>
    {!compact && <div className={styles.passportPhoto}><Image src="/images/2023-komatsu-wa475-10.jpg" alt="" fill sizes="260px" /></div>}
    <div className={styles.passportBody}><strong>{machine.year} {machine.make} {machine.model}</strong><small>{machine.hours} &nbsp; / &nbsp; {machine.location}</small><ul><li>IDENTITY</li><li>SPECS</li><li>OWNERSHIP</li><li>HISTORY</li></ul></div>
  </article>;
}
...
function MarketplaceVisual({ machine }) {
  return <div className={styles.marketplaceVisual}><PassportCard machine={machine} compact /><div className={styles.channelSpine} aria-hidden="true" /><div className={styles.channelList}>{CHANNELS.map(([name, detail]) => <div className={styles.channel} key={name}><i /><span><b>{name}</b><small>{detail}</small></span></div>)}</div></div>;
}
...
export default function ConnectedHomepage() {
  ...
  return <div className={styles.page}>
    ...
    <main>
      ...
      <BenefitSection number="01" product="MARKETPLACE" headline="DISTRIBUTE WITHOUT REBUILDING" body="Create the machine once. Publish it, share it, and move it everywhere." benefit="ONE RECORD. EVERY CHANNEL." href="/browse-v2" tone="marketTone" image="/images/ixi-homepage-bay-marketplace.webp"><MarketplaceVisual machine={machine} /></BenefitSection>
