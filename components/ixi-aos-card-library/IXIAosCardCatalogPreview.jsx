...
export default function IXIAosCardCatalogPreview({
  ...
  if (isLocation) {
    const Card =
      slug === "location-standard-003"
        ? IXIAosCard003Location
        : slug === "location-standard-002"
      ...
      <div className="location-preview">
        ...
        {face === 3 || face === 4 ? (
          <div className="variant-switch">
            <button className={financialMode === "owned" ? "active" : ""} onClick={() => setFinancialMode("owned")}>
              {face === 4 ? "F4-A · OWNED" : "F3-A · OWNED"}
            </button>
            <button className={financialMode === "leased" ? "active" : ""} onClick={() => setFinancialMode("leased")}>
  ...
  return (
    <div className="generic">
      ...
      />
      <style jsx>{`
        .generic { position: relative; display: flex; justify-content: center; }
        .preview-error { width: 298px; height: 471px; display: flex; align-items: center; justify-content: center; background: #101010; color: #ffc400; }
      `}</style>
    </div>
  );
}
