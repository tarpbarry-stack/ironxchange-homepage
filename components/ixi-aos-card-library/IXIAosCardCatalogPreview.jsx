...
function createPreviewObject({
  ...
  const fields =
    ...
    typeof sampleData.fields ===
      "object" &&
    !Array.isArray(sampleData.fields)
      ? sampleData.fields
      : {};
  ...
  return {
    objectId:
      clean(sampleData?.objectId) ||
      "aos-card-catalog-preview",

    entityId:
      clean(sampleData?.entityId) ||
    ...
    displayName:
      clean(sampleData?.displayName) ||
      clean(template?.label) ||
      "AOS OBJECT",

    status:
      clean(sampleData?.status) ||
      "active",
    ...
    relationships:
      Array.isArray(sampleData?.relationships)
        ? sampleData.relationships
        : [],

    infrastructure:
      Array.isArray(sampleData?.infrastructure)
        ? sampleData.infrastructure
        : [],

    media:
      Array.isArray(sampleData?.media)
        ? sampleData.media
        : [],
