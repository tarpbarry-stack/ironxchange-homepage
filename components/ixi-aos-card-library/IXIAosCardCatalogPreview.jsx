...
export default function IXIAosCardCatalogPreview({template = null,sampleData = {},projection = null,directItems = [],parentLabel = "",skinId = "ixi:skin:default",onSaveObject = null}) {
    ...
    return (
      <div className={`location-preview ${transactOpen?"transact-mode":"aos-mode"}`} style={{ width: transactOpen ? "298px" : `${consoleDepth * 298}px` }}>
        ...
        <div className="console-stage">
          {transactOpen ? <IXITransactObjectConsole object={financialObject} onClose={() => setTransactOpen(false)}/> : <IXIAosLocationObjectConsole templateSlug={slug} object={financialObject} projection={projection} objects={Array.isArray(directItems) ? directItems : []} ixiState={current} onIxiStateChange={update} onSaveObject={onSaveObject} onAddObject={() => {}} onHideObject={() => {}} onDeleteObject={() => {}} onRecall={() => {}} onBoard={() => {}} onReturn={() => {}} onExposeObject={() => {}} financialMode={financialMode} f2skin={f2skin} onF2SkinChange={setF2skin} primaryFace={face} onPrimaryFaceChange={setFace} onOpenTransact={() => setTransactOpen(true)}/>} 
        </div>
  ...
  return <div className="generic"><IXIAosObjectConsole object={object} objectId={object.objectId} projection={projection} objects={directItems} cardDefinition={definition} skinId={skinId} parentLabel={clean(parentLabel) || clean(template.librarySection) || "AOS"} ixiCardState={{}} updateIxiCardState={null} previewCardState={current} updatePreviewCardState={update} renderModule={null} studioEditing={false} selectedModuleId="" onSelectModule={null} onSelectFace={null} onCreateFace={null} enableCardScaling={false} cardScaleMode="xl"/><style jsx>{`.generic{position:relative;display:flex;justify-content:center}.preview-error{width:298px;height:471px;display:flex;align-items:center;justify-content:center;background:#101010;color:#ffc400}`}</style></div>;
}
