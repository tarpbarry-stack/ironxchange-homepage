...
const Inquiry=dynamic(()=>import("./SalesDeskInquiry"),{ssr:false});
...
export default function SalesDeskSurface({initial,dashboardClass,workspace:w}) {
  ...
  const [inquiry,setInquiry]=useState(false),[calendarFocus,setCalendarFocus]=useState(null);
  ...
  useEffect(()=>{
    ...
    const timer=setTimeout(()=>salesRequest(`records/${tab}?q=${encodeURIComponent(query)}${dueOnly ? `&dueBefore=${todayLocal()}&openOnly=true` : ""}&limit=100`,{signal:controller.signal}).then(result=>{if(seq===listSequence.current)setLists(current=>({...current,[tab]:result}));}).catch(e=>{if(e.name!=="AbortError")setListError(e.message);}).finally(()=>{if(seq===listSequence.current)setLoading(false);}),180);
    return()=>{clearTimeout(timer);controller.abort();};
  },[tab,query,revision,dueOnly]);
  useEffect(()=>{
  ...
  const refreshSummary=()=>salesRequest(`bootstrap?today=${todayLocal()}`).then(result=>{setSummary(result.summary);setTeam(result.team || []);}).catch(e=>w.setNotice(e.message));
  useEffect(()=>{
    try{activeId.current=sessionStorage.getItem(activeStorage) || "";}catch{}
    const restore=async()=>{const id=activeId.current;if(!id)return;const seq=++activeSequence.current;try{const result=await salesRequest(`records/deals/${id}`);if(seq===activeSequence.current && activeId.current===id)setActiveDeal(result.record);}catch(e){if(seq===activeSequence.current){setActiveDeal(null);if(e.status===404 || e.status===403){activeId.current="";try{sessionStorage.removeItem(activeStorage);}catch{}}w.setNotice(e.message);}}};
    restore();window.addEventListener("focus",restore);return()=>{activeSequence.current++;window.removeEventListener("focus",restore);};
  },[activeStorage]);
  const setWorkingDeal=deal=>{activeId.current=deal?.id || "";activeSequence.current++;setActiveDeal(deal);try{if(deal)sessionStorage.setItem(activeStorage,deal.id);else sessionStorage.removeItem(activeStorage);}catch{}};
  const changeTab=next=>{setDueOnly(false);setQuery("");setTab(next);setHidden(v=>({...v,right:false}));if(window.innerWidth<1000)setMobile("right");};
  ...
  const inquirySaved=result=>{
    setLists(current=>{const next={...current};for(const [kind,record] of [["contacts",result.contact],["deals",result.deal]]){const list=next[kind] || emptyList,exists=list.items.some(item=>item.id===record.id);next[kind]={...list,items:[record,...list.items.filter(item=>item.id!==record.id)],total:list.total+(exists ? 0 : 1)};}return next;});
    setRevision(n=>n+1);refreshSummary();
  };
  const inquiryCalendar=deal=>{setInquiry(false);setCalendarFocus({date:deal.dueDate,zone:deal.timeZone,assignedTo:deal.assignedTo});setPane("calendar");setMobile("");};
  const loadMore=async()=>{
    if(loading)return;const seq=++listSequence.current;setLoading(true);
    try{const result=await salesRequest(`records/${tab}?q=${encodeURIComponent(query)}${dueOnly ? `&dueBefore=${todayLocal()}&openOnly=true` : ""}&offset=${collection.items.length}&limit=100`);if(seq===listSequence.current)setLists(current=>({...current,[tab]:{...result,items:[...current[tab].items,...result.items]}}));}
  ...
  const showItem=item=>{
    ...
    if(tab==="deals"){activateDeal(item);return;}
    openEditor(tab,item);
  };
  ...
  return <div className={`${dashboardClass} ${styles.salesDesk}`} data-ixi-sales-desk="v3">
    ...
    <main className={`sales-workspace ${hidden.left ? "hide-machines" : ""} ${hidden.right ? "hide-sales" : ""} ${mobile ? `mobile-${mobile}` : ""}`}>
      ...
      <aside className="sales-work-rail" aria-label="Sales records"><div className="sales-rail-heading"><div><span className="sales-eyebrow">CUSTOMERS · CONVERSATIONS · NEXT ACTION</span><h2>YOUR SALES DESK</h2></div><button aria-label="Hide sales rail" onClick={()=>{setHidden(v=>({...v,right:true}));setMobile("");}}>›</button></div>
        ...
        <div className="sales-rail-tools"><input ref={searchRef} aria-label={`Search ${tab}`} placeholder={tab==="contacts" ? "Name, phone, company, interest…" : `Search ${tab}…`} value={query} onChange={e=>setQuery(e.target.value)}/>{["tasks","deals"].includes(tab) && <button className="sales-subtle" aria-pressed={dueOnly} onClick={()=>setDueOnly(v=>!v)}>{dueOnly ? "TODAY & OVERDUE · SHOW ALL" : "SHOW TODAY & OVERDUE"}</button>}<div className="sales-list-summary"><span>{loading ? "UPDATING…" : `${collection.total} ${tab.toUpperCase()}`}</span><button onClick={()=>openEditor(tab)} disabled={!canWrite || (tab==="boards" && !w.openMachines.length)}>+ {tab==="contacts" ? "CONTACT" : tab==="tasks" ? "FOLLOW-UP" : tab==="boards" ? "SAVE BOARD" : "DEAL"}</button></div></div>
