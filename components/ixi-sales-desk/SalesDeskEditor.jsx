...
import {scheduleDefaults} from "../../lib/sales-desk/calendar.mjs";
...
export default function SalesDeskEditor({editor,people,boardMachines=[],readOnly=false,actor={},team=[],onClose,onSaved,onQuote,onOpenMachines,onOpenLinked,onFollowUp,onPackage}) {
  const dialog=useRef(null),saveLock=useRef(false),command=useRef(null),scheduleReview=useRef("");
  const [value,setValue]=useState(()=>["tasks","deals"].includes(editor.kind) ? scheduleDefaults(editor.record) : editor.record),[dirty,setDirty]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState(""),[uncertain,setUncertain]=useState(false);
  const [contactQuery,setContactQuery]=useState(""),[contacts,setContacts]=useState([]),[notes,setNotes]=useState([]),[note,setNote]=useState("");
  const [history,setHistory]=useState(editor.history || []),[noteError,setNoteError]=useState(""),[conflicts,setConflicts]=useState([]);
  ...
  const patch=(key,next)=>{setValue(v=>({...v,[key]:next,...(key==="nextAction" && next!==v.nextAction ? {actionCompleted:false,actionOutcome:""} : {})}));setDirty(true);setError("");command.current=null;};
  const save=async e=>{
    ...
    try {
      if(scheduleReview.current!==command.current.commandId && ["tasks","deals"].includes(kind) && value.allDay===false && !value.completed && !value.actionCompleted && !value.canceled) {
        const preview=await salesRequest("calendar/preview",{body:{kind,record:value}});
        setConflicts(preview.conflicts);
        if(preview.conflicts.length && !window.confirm(`${preview.conflicts.length} overlapping commitment(s): ${preview.conflicts.map(c=>c.title).join(", ")}. Save this time anyway?`))return;
      }
      scheduleReview.current=command.current.commandId;
      const result=await salesRequest("commands",{body:command.current});setUncertain(false);setConflicts(result.conflicts || []);setValue(result.record);setDirty(false);command.current=null;onSaved(kind,result.record);
      try{const detail=await salesRequest(`records/${kind}/${result.record.id}`);setHistory(detail.history);}catch(e){setError(`Saved. Change history could not refresh: ${e.message}`);}
    }catch(e){const status=e.status || e.statusCode;const unknown=!status || status>=500;setUncertain(unknown);setError(unknown ? `${e.message} Retry SAVE to confirm the original request before changing these fields.` : e.message);}finally{saveLock.current=false;setBusy(false);}
  };
  ...
  return <dialog ref={dialog} className="sales-dialog" onCancel={e=>{e.preventDefault();close();}} aria-labelledby="sales-editor-title">
    ...
    <div className="sales-dialog-body"><form onSubmit={save}>
      <fieldset disabled={busy || readOnly || uncertain} className="sales-fields">
        {kind==="contacts" && <>
        ...
        {["deals","tasks"].includes(kind) && <SalesScheduleFields value={value} patch={patch} deal={kind==="deals"}/>}
        ...
        {["contacts","deals","tasks"].includes(kind) && <Field label="ASSIGNED TO" wide><select disabled={!actor.canAssign} value={value.assignedTo ?? actor.actorId ?? ""} onChange={e=>patch("assignedTo",e.target.value)}>{actor.canAssign && <option value="">UNASSIGNED</option>}{value.assignedTo && !team.some(m=>m.principalId===value.assignedTo) && <option value={value.assignedTo}>Previous assignee · choose an active member</option>}{team.map(member=><option key={member.principalId} value={member.principalId}>{member.name}</option>)}</select></Field>}
      </fieldset>
      {conflicts.length>0 && <div className="sales-error" role="status"><strong>OVERLAPPING COMMITMENTS</strong>{conflicts.map((c,i)=><p key={i}>{c.title}</p>)}</div>}
    ...
    {value.id && kind==="tasks" && value.dealId && <div className="sales-actions"><button disabled={dirty || busy} onClick={()=>onOpenMachines({id:value.dealId})}>OPEN DEAL MACHINES ↗</button></div>}
