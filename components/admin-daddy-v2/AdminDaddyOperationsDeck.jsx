...
export default function AdminDaddyOperationsDeck() {
  const [data, setData] = useState({ jobs:null, integrity:null, acquisition:null, media:null, marketplace:null });
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      getJson("/api/admin-daddy/marketplace")
    ]).then(results => {
      if (cancelled) return;
      const next = { jobs:null, integrity:null, acquisition:null, media:null, marketplace:null };
      ["jobs","integrity","acquisition","media","marketplace"].forEach((key,index) => {
  ...
  const marketSummary = data.marketplace?.summary || {};
  ...
  return <section className="deck">
    ...
    <div className="grid">
      ...
      <article><h3>MARKETPLACE RECONCILIATION</h3><div className="metrics"><Metric label="LISTINGS" value={data.marketplace?.listings ?? data.marketplace?.total}/><Metric label="CONFLICTS" value={data.marketplace?.conflictCount}/><Metric label="CRITICAL" value={marketSummary.critical}/><Metric label="ACTION" value={marketSummary.action}/></div><small className="state">{data.marketplace?.live ? "LIVE LISTINGS PROJECTION" : "PROJECTION PENDING"}</small></article>
    </div>
    ...
    <style jsx>{`
      .deck{margin:0;background:#07090a;color:#edf0f2;font-family:Arial,sans-serif;border-top:2px solid #242a2e;padding:10px}.deckHead{height:36px;display:flex;align-items:center;justify-content:space-between;padding:0 12px;border:1px solid #3a4044;background:linear-gradient(#1a1f23,#0e1214);font-size:10px}.deckHead b{color:#f0b600}.deckHead span{color:#77838b;font-size:8px}.grid{display:grid;grid-template-columns:repeat(5,1fr);gap:8px;margin-top:8px}.grid article{border:1px solid #343a3f;background:linear-gradient(145deg,#151a1e,#0b0e11);min-height:126px}.grid h3{margin:0;padding:9px 10px;border-bottom:1px solid #30363a;color:#d9dde0;font-size:9px}.metrics{display:grid;grid-template-columns:repeat(2,1fr)}.metric{padding:10px;border-right:1px solid #242a2e;border-bottom:1px solid #242a2e}.metric small,.metric span{display:block;color:#78838a;font-size:7px}.metric strong{display:block;font-size:17px;margin:5px 0;color:#fff}.state{display:block;padding:6px 10px;color:#7d8990;font-size:7px}.search{margin-top:8px;padding:10px;border:1px solid #343a3f;background:#0c1012}.search label{display:block;color:#f0b600;font-size:8px;margin-bottom:7px}.search>div{display:grid;grid-template-columns:1fr 110px;gap:6px}.search input{background:#07090a;color:#fff;border:1px solid #343a3f;padding:10px;font-size:10px}.search button{background:linear-gradient(#302b14,#171710);color:#f0b600;border:1px solid #806b17;font-size:9px;font-weight:800}.results{display:grid;grid-template-columns:repeat(4,1fr);gap:5px;margin-top:6px}.results>div{border:1px solid #30363a;background:#0d1113;padding:8px}.results b,.results span,.results small{display:block}.results b{font-size:9px}.results span{font-size:7px;color:#f0b600;margin-top:4px}.results small{font-size:7px;color:#7f8a91;margin-top:3px}@media(max-width:1250px){.grid{grid-template-columns:repeat(3,1fr)}}@media(max-width:1050px){.grid,.results{grid-template-columns:repeat(2,1fr)}}@media(max-width:650px){.grid,.results{grid-template-columns:1fr}.search>div{grid-template-columns:1fr}}
    `}</style>
  </section>;
}
