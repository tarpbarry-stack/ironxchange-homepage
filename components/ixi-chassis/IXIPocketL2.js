...
export default function IXIPocketL2({
  leftPocket2Mode,
  machineContainers,
  armedDestination,
  WorkspaceDropPad,
...
/>
  ...
  aria-label="Send Pocket III machines to IXI Theater"
...
/>
  ...
  aria-label="Move Pocket III machines to the bottom active stack"
...
/>
    <button
      ...
      aria-label="Return Pocket III machines to the board"
      data-label="BOARD"
      onClick={(e) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    <button
      ...
      aria-label="Send Pocket III machines"
      title="Email the front machine Passport"
      data-label="SEND"
      disabled={(machineContainers.pocketLeft2 || []).length === 0}
      onClick={(e) => {
        e.preventDefault();
        e.stopPropagation();
        openIXIPassportEmail(
          getPocketFrontMachine({
            machineIds: machineContainers.pocketLeft2 || [],
            getListingById
          })
        );
      }}
