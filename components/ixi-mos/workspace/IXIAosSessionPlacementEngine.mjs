...
function canonicalObjectId(value) {
  const id = required(value, "objectId");
  if (!/^object_/.test(id)) {
    throw new Error("Session placement accepts canonical objectId values only.");
  }
  return id;
}
...
function normalizeOrigin({
  ...
  const mode = [PREVIEW, OPERATING, TUCKED].includes(admissionMode)
    ? admissionMode
    : PREVIEW;
...
export function validateAosSessionPlacement(state) {
  ...
  Object.entries(state.objectsById).forEach(([key, record]) => {
    ...
    canonicalObjectId(record?.objectId);
    if (![PREVIEW, OPERATING, TUCKED].includes(record.sessionOrigin?.admissionMode)) {
    ...
    if (![PREVIEW, OPERATING, TUCKED].includes(record.operatingState)) {
