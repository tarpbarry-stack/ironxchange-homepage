...
export const IXI_TRANSACT_DASHBOARD_INCLUDES=Object.freeze(["executive","attention","ar","ap","treasury","gl-controls","reporting","operations","work-orders","purchase-orders","assets","rental"]);
const WORKSPACE_INCLUDES=Object.freeze({
 executive:["executive","attention","ar","ap","treasury","gl-controls","reporting"],
 ar:["ar","attention"],invoices:["ar"],sales:["ar","reporting"],
 ap:["ap","attention"],bills:["ap","attention"],"purchase-orders":["purchase-orders","attention"],
 treasury:["treasury","attention"],reconciliation:["treasury","attention"],
 gl:["gl-controls","attention"],close:["gl-controls","attention"],reporting:["reporting"],profitability:["reporting"],
 "work-orders":["operations","work-orders","attention"],
 assets:["assets","reporting","attention"],
 service:["operations","work-orders","ar","attention"],
 rental:["rental","ar","ap","attention"]
});
...
export function createIXITransactDashboardQuery({scope={},period={},currency="USD",filters={},include=IXI_TRANSACT_DASHBOARD_INCLUDES}={}){const normalizedInclude=Array.from(new Set(safeArray(include).map(clean).filter(value=>IXI_TRANSACT_DASHBOARD_INCLUDES.includes(value))));return{contract:"ixi-transact-dashboard-query",contractVersion:"1.3.0",scope:normalizeIXITransactDashboardScope(scope),period:normalizeIXITransactDashboardPeriod(period),currency:/^[A-Z]{3}$/.test(clean(currency).toUpperCase())?clean(currency).toUpperCase():"USD",filters:safeObject(filters),include:normalizedInclude.length?normalizedInclude:[...IXI_TRANSACT_DASHBOARD_INCLUDES]};}
