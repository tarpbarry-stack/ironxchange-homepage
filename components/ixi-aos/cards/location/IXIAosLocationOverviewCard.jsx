...
export default function IXIAosLocationOverviewCard({
  ...
  return (
    <article className={`ixi-generic-overview gov-${variant}`} data-card-number={variant}>
      ...
      <main className="gov-body">
        ...
        {variant !== "002" ? (
          <div className="gov-preview">
            <strong title={collectionTitle}>{collectionTitle}</strong>
            <span>{activeChild ? `${activeIndex + 1}/${children.length}` : "0/0"}</span>
            <button type="button" disabled={!activeChild} onClick={event => { event.stopPropagation(); if (activeChild) onExposeObject?.(activeChild, runtimeObject); }}>OUT ↗</button>
          </div>
        ) : null}
      ...
      <style jsx global>{`
        .ixi-generic-overview,.ixi-generic-overview *{box-sizing:border-box}.ixi-generic-overview{--y:#ffc400;--line:#343a35;--soft:#252a26;position:relative;width:${W}px;height:${H}px;overflow:hidden;border:1px solid #454b47;border-radius:13px;background:linear-gradient(180deg,#101310,#080a09);color:#f4f5f4;font-family:Arial,Helvetica,sans-serif;box-shadow:inset 0 1px #ffffff12,0 18px 40px #0008}.gov-head{position:absolute;inset:0 0 auto;height:${HEADER}px;padding:7px 10px;border-bottom:1px solid #303531;background:linear-gradient(180deg,#171a18,#101210);z-index:40}.gov-identity{width:185px;min-width:0}.gov-identity>span{display:block;color:var(--y);font-size:6px;font-weight:950;letter-spacing:.08em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.gov-identity>strong{display:block;margin-top:4px;overflow:hidden;color:#f6f7f6;font-size:14px;font-weight:950;line-height:1;text-overflow:ellipsis;white-space:nowrap}.gov-body{position:absolute;top:${HEADER}px;left:7px;right:7px;display:flex;flex-direction:column;min-height:0;gap:5px;padding:5px 0;overflow:hidden}.gov-001 .gov-body,.gov-003 .gov-body{bottom:${RAIL + THUMBS + COMMANDS + 7}px}.gov-002 .gov-body{bottom:${RAIL + COMMANDS + 7}px}.gov-media{flex:0 0 112px;overflow:hidden;border:1px solid var(--line);border-radius:5px;background:#0b0e0c}.gov-media.compact{flex-basis:84px}.gov-media img{width:100%;height:100%;object-fit:cover;display:block}.gov-media-empty{height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:5px;color:#69716c;background:linear-gradient(180deg,#111411,#0c0f0d)}.gov-media-empty b{font-size:21px;letter-spacing:.08em;color:#7d857f}.gov-media-empty span{font-size:5px;font-weight:900;letter-spacing:.12em}.gov-descriptor{flex:0 0 43px;display:grid;grid-template-columns:20px 1fr;align-items:center;padding:0 8px;border:1px solid var(--line);border-radius:5px;background:#111411}.gov-002 .gov-descriptor{flex-basis:57px}.gov-mark{color:var(--y);font-size:12px;text-align:center}.gov-descriptor strong{display:block;color:#eef1ef;font-size:8px;font-weight:900;line-height:1.15;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.gov-descriptor small{display:block;margin-top:3px;color:#969d98;font-size:6px;font-weight:800;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.gov-preview{flex:0 0 22px;display:grid;grid-template-columns:minmax(0,1fr) 34px 52px;align-items:center;border:1px solid var(--line);border-radius:5px;background:#101310;overflow:hidden}.gov-preview>strong{min-width:0;overflow:hidden;padding:0 8px;color:#e8ebe9;font-size:7px;font-weight:900;text-overflow:ellipsis;white-space:nowrap}.gov-preview>span{color:#7c857f;font-size:6px;text-align:center}.gov-preview>button{height:100%;border:0;border-left:1px solid var(--soft);background:transparent;color:var(--y);font-size:7px;font-weight:950}.gov-preview>button:disabled{opacity:.25}.gov-metrics{flex:0 0 43px;display:grid;grid-template-columns:1fr 1.25fr 1fr;gap:4px}.gov-002 .gov-metrics{flex-basis:48px}.gov-metric{min-width:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;border:1px solid var(--line);border-radius:5px;background:#111411}.gov-metric span{max-width:100%;padding:0 3px;overflow:hidden;color:#969d98;font-size:5px;font-weight:900;letter-spacing:.05em;text-overflow:ellipsis;white-space:nowrap}.gov-metric strong{color:#f6f7f6;font-size:12px;font-weight:950;line-height:1}.gov-relations{min-height:0;overflow:hidden;border:1px solid var(--line);border-radius:5px;background:#101310;flex:1}.gov-relations h3{height:19px;margin:0;display:flex;align-items:center;padding:0 6px;border-bottom:1px solid var(--soft);color:var(--y);font-size:6px;font-weight:950;letter-spacing:.05em}.gov-relation-scroll{height:calc(100% - 19px);overflow-y:auto;scrollbar-width:thin;scrollbar-color:#555c57 #101310}.gov-relation-scroll button{width:100%;min-height:29px;display:grid;grid-template-columns:1fr 18px;align-items:center;padding:4px 6px 4px 8px;border:0;border-bottom:1px solid #242925;background:#101310;color:#fff;text-align:left}.gov-relation-scroll button:nth-child(even){background:#121612}.gov-relation-scroll small{display:block;color:#8f9892;font-size:5px;font-weight:900}.gov-relation-scroll strong{display:block;margin-top:2px;color:#f1f3f2;font-size:7px;font-weight:900}.gov-relation-scroll em{display:block;color:#737c76;font-size:5px;font-style:normal}.gov-relation-scroll button>b{color:var(--y);font-size:10px;text-align:center}.gov-empty{padding:20px;color:#747d77;font-size:6px;font-weight:900;text-align:center}.gov-commands{position:absolute;left:7px;right:7px;height:${COMMANDS}px;display:grid;grid-template-columns:repeat(3,1fr);border:1px solid var(--line);border-radius:5px;background:#101310;overflow:hidden;z-index:20}.gov-001 .gov-commands,.gov-003 .gov-commands{bottom:${RAIL + THUMBS + 4}px}.gov-002 .gov-commands{bottom:${RAIL + 3}px}.gov-commands button{border:0;border-right:1px solid var(--soft);background:transparent;color:#b7beb9;font-size:7px;font-weight:900}.gov-commands button:last-child{border-right:0}.gov-commands button:hover{color:var(--y);background:#151915}.gov-commands button span{margin-left:3px}.gov-thumbs{position:absolute;left:7px;right:7px;bottom:${RAIL}px;height:${THUMBS}px;border:1px solid var(--line);border-radius:5px 5px 0 0;background:#0b0e0c;z-index:18;overflow:hidden}.gov-editor{position:absolute;inset:0 0 16px;background:#0a0d0b;z-index:100}.gov-editor-head{height:43px;display:flex;align-items:center;justify-content:space-between;padding:7px 9px;border-bottom:1px solid #303532;background:linear-gradient(180deg,#171a18,#101210)}.gov-editor-head small{display:block;color:#8c958f;font-size:5px;font-weight:900}.gov-editor-head strong{display:block;margin-top:3px;font-size:10px}.gov-editor-head nav{display:flex}.gov-editor-head button{height:24px;padding:0 8px;border:1px solid #3b423d;background:#101310;color:#dfe3e0;font-size:6px;font-weight:900}.gov-editor-head button:first-child{color:var(--y)}.gov-editor-scroll{position:absolute;top:43px;left:0;right:0;bottom:0;padding:9px;overflow-y:auto}.gov-editor-scroll label{display:block;margin-bottom:7px}.gov-editor-scroll label span{display:block;margin-bottom:3px;color:#8d9690;font-size:5px;font-weight:900}.gov-editor-scroll input{width:100%;height:28px;padding:0 7px;border:1px solid #363d38;border-radius:4px;background:#111512;color:#fff;font-size:8px;font-weight:800;outline:none}
      `}</style>
    </article>
  );
}
