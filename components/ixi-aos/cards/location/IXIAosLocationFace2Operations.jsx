...
function FieldRow({ icon = "•", label, value, editing = false, onChange = null }) {
  return (
    <div className="ops-field-row">
      ...
      {onChange ? <button type="button" className="ops-edit-button">EDIT</button> : <span />}
    </div>
  );
}
...
export default function IXIAosLocationFace2Operations({
  ...
  const relationshipItems = ["SHOP", "LOADING AREA", "GATE", "FORKLIFT"];
    ...
    <div className="ixi-aos-location-f2 card board-color-none board-outline-1">
      ...
      <div className="ops-scroll-body">
        ...
        <Section title="ARRIVAL">
          <FieldRow icon="⌖" label="ADDRESS" value={address} />
      ...
      <style jsx>{`
        ...
        .ops-identity { min-width: 0; flex: 1; }
        .ops-identity span { display: block; color: var(--f2-accent); font-size: 6.4px; font-weight: 950; letter-spacing: .06em; }
        .ops-identity strong { display: block; margin-top: 4px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 15px; line-height: 1; font-weight: 950; }
        ...
        .ops-scroll-body::-webkit-scrollbar { width: 4px; }
        .ops-scroll-body::-webkit-scrollbar-track { background: transparent; }
        .ops-scroll-body::-webkit-scrollbar-thumb { background: rgba(255,255,255,.14); border-radius: 999px; }

        .gate-code-row { height: 35px; display: grid; grid-template-columns: 72px 1fr 40px; align-items: center; padding: 0 7px; margin-bottom: 6px; border: 1px solid rgba(255,196,0,.38); border-radius: 5px; background: rgba(255,196,0,.025); }
        .gate-code-row span { color: var(--f2-accent); font-size: 6.5px; font-weight: 950; }
        .gate-code-row strong { font-size: 17px; letter-spacing: .06em; }
        .gate-code-row button,
        .ops-edit-button,
        .site-instructions button { height: 19px; border: 1px solid var(--f2-border); border-radius: 3px; background: #0b0c0c; color: rgba(255,255,255,.72); font-size: 6px; font-weight: 950; cursor: pointer; }

        .ops-section { margin-bottom: 6px; border: 1px solid var(--f2-border-soft); border-radius: 5px; background: var(--f2-panel-2); overflow: hidden; }
        .ops-section-title { height: 20px; display: flex; align-items: center; padding: 0 7px; color: var(--f2-accent); font-size: 6.4px; font-weight: 950; letter-spacing: .065em; border-bottom: 1px solid var(--f2-border-soft); }
        .ops-section-body { padding: 3px; }

        .ops-field-row { min-height: 23px; display: grid; grid-template-columns: 18px 92px minmax(0,1fr) 34px; align-items: center; border-bottom: 1px solid rgba(255,255,255,.04); }
        .ops-field-row:last-child { border-bottom: 0; }
        .ops-icon { font-size: 8px; text-align: center; }
        .ops-label { font-size: 6px; font-weight: 850; color: rgba(255,255,255,.68); }
        .ops-field-row strong { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 6.6px; font-weight: 900; }
        .ops-field-row input { min-width: 0; width: 100%; height: 18px; border: 1px solid rgba(255,196,0,.35); border-radius: 3px; background: #070808; color: #fff; font-size: 6.5px; padding: 0 4px; }
        .ops-edit-button { height: 16px; font-size: 5.5px; }

        .ops-grid { display: grid; gap: 2px; }
        .ops-grid.four { grid-template-columns: repeat(4, minmax(0,1fr)); }
        .ops-grid.two { grid-template-columns: repeat(2, minmax(0,1fr)); }
        .ops-tile { min-height: 39px; display: grid; grid-template-columns: 15px minmax(0,1fr); grid-template-rows: auto auto; align-items: center; padding: 4px; border: 1px solid rgba(255,255,255,.055); border-radius: 3px; background: rgba(255,255,255,.018); overflow: hidden; }
        .ops-tile-icon { grid-row: 1 / span 2; font-size: 8px; }
        .ops-tile-label { min-width: 0; font-size: 5.2px; font-weight: 850; color: rgba(255,255,255,.66); line-height: 1.15; overflow-wrap: anywhere; }
        .ops-tile strong { min-width: 0; color: #f2f2ef; font-size: 6.4px; font-weight: 950; line-height: 1.1; overflow-wrap: anywhere; }
        .ops-tile.emphasis strong { color: var(--f2-good); }
        .ops-grid.compact .ops-tile { min-height: 36px; }

        .ops-wide-fact { height: 23px; margin-top: 2px; padding: 0 7px; display: flex; align-items: center; justify-content: space-between; border: 1px solid rgba(255,255,255,.05); border-radius: 3px; }
        .ops-wide-fact span { font-size: 5.7px; font-weight: 850; }
        .ops-wide-fact strong { font-size: 6.5px; }
        .ops-chip-row { display: flex; gap: 5px; flex-wrap: wrap; padding: 5px 4px 2px; }
        .ops-chip-row span { font-size: 5.3px; font-weight: 850; color: rgba(255,255,255,.62); }

        .site-instructions { min-height: 42px; display: grid; grid-template-columns: 18px minmax(0,1fr) 34px; gap: 3px; align-items: center; padding: 4px; }
        .site-instructions > span { color: var(--f2-accent); text-align: center; }
        .site-instructions strong { font-size: 6px; line-height: 1.35; font-weight: 850; }
        .site-instructions textarea { width: 100%; min-height: 34px; resize: vertical; border: 1px solid rgba(255,196,0,.35); border-radius: 3px; background: #070808; color: #fff; font-size: 6px; padding: 4px; outline: none; }

        .relationship-row { display: grid; grid-template-columns: repeat(4,minmax(0,1fr)); gap: 3px; }
        .relationship-row button { height: 25px; display: flex; align-items: center; justify-content: space-between; padding: 0 6px; border: 1px solid rgba(255,255,255,.08); border-radius: 3px; background: rgba(255,255,255,.018); color: rgba(255,255,255,.82); font-size: 5.8px; font-weight: 900; }
        .relationship-row button span { color: var(--f2-cyan); font-size: 10px; }
      `}</style>
    </div>
  );
}
