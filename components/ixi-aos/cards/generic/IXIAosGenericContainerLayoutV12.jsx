...
function MetricRow({ label, value, maximum }) {
  ...
  return (
    <div className="gcv12-metric-row">
      <span>{label}</span>
...
function ScrollSection({ title, children, className = "" }) {
  return (
    <section className={`gcv12-section ${className}`.trim()}>
      <div className="gcv12-section-title">{title}</div>
      <div className="gcv12-section-scroll">{children}</div>
    </section>
  );
}
...
function GenericEditor({ object, saving, onCancel, onSave }) {
  ...
  async function save() {
    ...
    definitions.forEach(definition => {
      nextFields[definition.fieldId] = parseEditedValue(definition, draft[definition.fieldId]);
    });
    const nextObject = {
      ...object,
    ...
    await onSave?.(nextObject);
  }

  return (
    <div className="gcv12-editor" onPointerDown={event => event.stopPropagation()}>
      <div className="gcv12-editor-head">
      ...
      <div className="gcv12-editor-scroll">
...
export default function IXIAosGenericContainerLayoutV12({
  ...
  function renderHero() {
    ...
    if (variant === 2) {
      return (
        <div className="gcv12-kpis">
          <div className="gcv12-kpi"><small>{totalLabel}</small><strong>{items.length}</strong></div>
          {entries.slice(0, 3).map(entry => <div className="gcv12-kpi" key={entry.label}><small>{entry.label}</small><strong>{entry.value}</strong></div>)}
        </div>
      );
    }
    ...
    if (variant === 3) {
      return (
        <ScrollSection title={heroGroup?.label || emptyAggregateTitle} className="gcv12-status-section">
          <div className="gcv12-status-grid">
            {entries.map(entry => <div className="gcv12-status-tile" key={entry.label}><span>{icon}</span><small>{entry.label}</small><strong>{entry.value}</strong></div>)}
            {!entries.length ? <div className="gcv12-empty">NO AGGREGATE DATA</div> : null}
          </div>
        </ScrollSection>
      );
    }
    ...
    return (
      <div className="gcv12-hero">
        <div className="gcv12-hero-mark"><b>IXI</b><span>{icon}</span></div>
        <div className="gcv12-hero-total"><small>{totalLabel}</small><strong>{items.length}</strong></div>
        <div className="gcv12-hero-values">{entries.slice(0, 2).map(entry => <div key={entry.label}><small>{entry.label}</small><strong>{entry.value}</strong></div>)}</div>
      </div>
    );
  }

  return (
    <article className="ixi-generic-container-v12" data-card-variant={variant} data-card-skin={skinId}>
      <header className="gcv12-header">
        <div className="gcv12-identity"><span>{getObjectLabel(runtimeObject)}</span><h2>{getObjectDisplayName(runtimeObject)}</h2></div>
        {!editing ? (
      ...
      <main className="gcv12-body">
        {renderHero()}

        {(variant !== 3 ? [heroGroup, ...secondaryGroups] : secondaryGroups)
          .filter(Boolean)
          .map(group => {
            const maximum = Math.max(1, ...group.entries.map(entry => entry.value));
            return (
              <ScrollSection key={group.groupId} title={group.label}>
                {group.entries.map(entry => <MetricRow key={entry.label} label={entry.label} value={entry.value} maximum={maximum} />)}
              </ScrollSection>
            );
          })}

        {!aggregateGroups.length ? (
          <ScrollSection title={emptyAggregateTitle}><div className="gcv12-empty">NO DECLARED AGGREGATES</div></ScrollSection>
        ) : null}

        <ScrollSection title={relationshipsTitle} className="gcv12-relationships">
          {relationships.map(relationship => (
            <button key={relationship.id} type="button" className="gcv12-relationship" onClick={event => event.stopPropagation()}>
          ...
          {!relationships.length ? <div className="gcv12-empty">NO RELATIONSHIPS</div> : null}
        </ScrollSection>
      </main>

      <nav className="gcv12-commands">
