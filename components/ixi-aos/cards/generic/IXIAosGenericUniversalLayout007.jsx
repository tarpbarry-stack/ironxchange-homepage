...
function fileToMedia(file) {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => resolve({ url: String(reader.result || ""), name: clean(file?.name), type: clean(file?.type), size: Number(file?.size || 0), source: "object-media-upload" });
...
function UniversalEditor({ object, saving, onCancel, onSave }) {
  ...
  useEffect(() => {
    ...
    const nextDraft = {};
    nextDefinitions.forEach(definition => { nextDraft[definition.fieldId] = inputValue(getObjectFields(object)?.[definition.fieldId]); });
    setDraft(nextDraft);
  }, [object]);
  ...
  function removeField(fieldId) {
    ...
    setDraft(current => { const next = { ...current }; delete next[fieldId]; return next; });
  }
  ...
  async function save() {
    ...
    Object.keys(nextFields).forEach(fieldId => { if (fieldId.startsWith("field_") && !retainedIds.has(fieldId)) delete nextFields[fieldId]; });
    normalizedDefinitions.forEach(definition => { nextFields[definition.fieldId] = parseValue(definition, draft[definition.fieldId]); });
    await onSave?.({ ...object, displayName: clean(name) || getObjectDisplayName(object), fields: nextFields, media, fieldDefinitions: normalizedDefinitions, metadata: { ...(object?.metadata || {}), fieldDefinitions: normalizedDefinitions } });
  }

  return <div className="u007-editor" onPointerDown={event => event.stopPropagation()}>
    <div className="u007-editor-head"><div><small>{getObjectLabel(object)}</small><strong>EDIT OBJECT</strong></div><nav><button type="button" disabled={saving} onClick={save}>SAVE</button><button type="button" disabled={saving} onClick={onCancel}>CANCEL</button></nav></div>
    <div className="u007-editor-scroll">
      <IXIAosPrimaryMediaEditor media={media} onChange={setMedia}/>
      <section><div className="u007-editor-title">IDENTITY</div><label className="u007-name-field"><span>OBJECT NAME</span><input value={name} onChange={event => setName(event.target.value)}/></label></section>
      <section><div className="u007-editor-title">DETAIL FIELDS</div><div className="u007-editor-columns"><span>LABEL</span><span>VALUE</span><i/></div>
        {definitions.map((definition,index)=><div className="u007-editor-row" key={definition.fieldId}><input aria-label={`Field ${index+1} label`} value={definition.label} onChange={event=>setDefinitions(current=>current.map(item=>item.fieldId===definition.fieldId?{...item,label:event.target.value}:item))}/><input aria-label={`${definition.label} value`} value={draft[definition.fieldId]??""} onChange={event=>setDraft(current=>({...current,[definition.fieldId]:event.target.value}))}/><button type="button" onClick={()=>removeField(definition.fieldId)}>×</button></div>)}
...
export default function IXIAosGenericUniversalLayout007({ object={}, children=[], ixiState={}, onSaveObject=null, onAddObject=null, onHideObject=null, onDeleteObject=null, onOpenConsole=null, onOpenTransact=null, onRecall=null, onBoard=null, onReturn=null, onExposeObject=null, onSendFront=null, onSendBack=null, onCycleColor=null, onCycleOutline=null, onCycleFace=null, onRailSend=null, armedDestination="", onSendToArmedDestination=null, skinId="v12", onSkinChange=null }) {
  ...
  const populatedFields=definitions.map(definition=>({definition,value:inputValue(fields?.[definition.fieldId])})).filter(item=>clean(item.value));
  ...
  async function addPrimaryPhoto(event){const file=event?.target?.files?.[0];if(!file||!file.type?.startsWith("image/"))return;const photo=await fileToMedia(file);const existing=asArray(runtimeObject?.media).slice(1);await save({...runtimeObject,media:[photo,...existing]});event.target.value="";}
  function command(event,callback){event.preventDefault();event.stopPropagation();callback?.(runtimeObject);}
  function openPhotoPicker(event){event.preventDefault();event.stopPropagation();mediaInputRef.current?.click();}

  return <article className="ixi-universal-card-007" data-card-number="007" data-card-skin={skinId}>
    ...
    <main className="u007-body">
      <section className="u007-media-shell">{image?<img src={image} alt={getObjectDisplayName(runtimeObject)}/>:<div className="u007-media-empty"><b>IXI</b><span>PRIMARY MEDIA</span></div>}{actions.canEdit?<button className="u007-media-action" type="button" disabled={saving} onClick={openPhotoPicker}>{image?"CHANGE PHOTO":"+ ADD PHOTO"}</button>:null}<input ref={mediaInputRef} className="u007-media-input" type="file" accept="image/*" onChange={addPrimaryPhoto}/></section>

      <section className="u007-section u007-details"><div className="u007-section-title">{detailsTitle}</div><div className="u007-section-scroll">{populatedFields.map(({definition,value})=><div className="u007-detail-row" key={definition.fieldId}><span>{definition.label}</span><strong>{value}</strong></div>)}{!populatedFields.length?<div className="u007-empty">NO DETAILS YET · EDIT TO ADD</div>:null}</div></section>

      <section className="u007-section u007-relationships"><div className="u007-section-title">{relationshipsTitle}</div><div className="u007-section-scroll">{relationships.map(relationship=><button type="button" className="u007-relationship-row" key={relationship.id} onClick={event=>event.stopPropagation()}><span><small>{relationship.label}</small><strong>{relationship.value}</strong>{relationship.secondary?<em>{relationship.secondary}</em>:null}</span><b>›</b></button>)}{!relationships.length?<div className="u007-empty">NO RELATIONSHIPS</div>:null}</div></section>
    </main>
    ...
    <style jsx global>{`
      ...
      .u007-body{position:absolute;top:43px;left:7px;right:7px;bottom:111px;display:flex;flex-direction:column;gap:5px;min-height:0;padding:5px 0;overflow-y:auto;scrollbar-width:thin;scrollbar-color:#4b514d #0b0e0c}.u007-body::-webkit-scrollbar,.u007-section-scroll::-webkit-scrollbar,.u007-editor-scroll::-webkit-scrollbar{width:5px;height:5px}.u007-body::-webkit-scrollbar-track,.u007-section-scroll::-webkit-scrollbar-track,.u007-editor-scroll::-webkit-scrollbar-track{background:#0b0e0c}.u007-body::-webkit-scrollbar-thumb,.u007-section-scroll::-webkit-scrollbar-thumb,.u007-editor-scroll::-webkit-scrollbar-thumb{background:#4b514d;border:1px solid #171b18;border-radius:999px}.u007-body::-webkit-scrollbar-thumb:hover,.u007-section-scroll::-webkit-scrollbar-thumb:hover,.u007-editor-scroll::-webkit-scrollbar-thumb:hover{background:#626a64}
      .u007-media-shell{position:relative;flex:0 0 112px;width:100%;overflow:hidden;border:1px solid var(--line);border-radius:5px;background:#0b0e0c}.u007-media-shell img{display:block;width:100%;height:100%;object-fit:cover;object-position:center;background:#080b09}.u007-media-empty{height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:5px;color:#69716c;background:#0d100e}.u007-media-empty b{color:#7d857f;font-size:21px;font-weight:950;letter-spacing:.03em}.u007-media-empty span{color:#747c76;font-size:5px;font-weight:900;letter-spacing:.08em}.u007-media-action{position:absolute;right:7px;bottom:7px;height:22px;padding:0 8px;border:1px solid #ffc40066;border-radius:4px;background:#0b0e0dda;color:var(--y);font-size:5.5px;font-weight:950;box-shadow:0 4px 10px #0008}.u007-media-action:hover{background:#171508}.u007-media-input{display:none}
      .u007-section{flex:0 0 auto;overflow:hidden;border:1px solid var(--line);border-radius:5px;background:#101310}.u007-details{height:118px;min-height:118px}.u007-relationships{height:92px;min-height:92px}.u007-section-title{height:20px;display:flex;align-items:center;padding:0 7px;border-bottom:1px solid var(--soft);background:#151916;color:var(--y);font-size:6px;font-weight:950}.u007-section-scroll{height:calc(100% - 20px);overflow-y:auto;scrollbar-width:thin;scrollbar-color:#4b514d #101310}.u007-detail-row{min-height:22px;display:grid;grid-template-columns:minmax(0,92px) 1fr;align-items:center;gap:7px;padding:4px 7px;border-bottom:1px solid #242925}.u007-detail-row:nth-child(even),.u007-relationship-row:nth-child(even){background:#ffffff08}.u007-detail-row span{overflow:hidden;color:#929a95;font-size:5.5px;font-weight:900;text-overflow:ellipsis;white-space:nowrap}.u007-detail-row strong{overflow:hidden;color:#eef1ef;font-size:7.5px;font-weight:900;text-align:right;text-overflow:ellipsis;white-space:nowrap}.u007-relationship-row{width:100%;min-height:27px;display:grid;grid-template-columns:1fr 18px;align-items:center;padding:3px 6px 3px 8px;border:0;border-bottom:1px solid #242925;background:transparent;color:#fff;text-align:left}.u007-relationship-row small{display:block;color:#8f9792;font-size:5px;font-weight:900}.u007-relationship-row strong{display:block;margin-top:1px;font-size:7px;font-weight:900}.u007-relationship-row em{display:block;color:#6e7771;font-size:5px;font-style:normal}.u007-relationship-row>b{color:var(--y);font-size:10px;text-align:center}.u007-empty{min-height:42px;display:grid;place-items:center;padding:8px;color:#68716b;font-size:5.5px;font-weight:900;text-align:center}
      .u007-commands{position:absolute;left:7px;right:7px;bottom:78px;height:27px;display:grid;grid-template-columns:repeat(3,1fr);overflow:hidden;border:1px solid var(--line);border-radius:5px;background:#0f120f}.u007-commands button{border:0;border-right:1px solid var(--soft);background:transparent;color:#b9c0bb;font-size:7px;font-weight:900}.u007-commands button:last-child{border-right:0}.u007-commands b{margin-left:3px;font-size:6px}.u007-child-rail{position:absolute;left:0;right:0;bottom:19px;height:55px;overflow:hidden;border-top:1px solid #292e2a;background:#080a09}
