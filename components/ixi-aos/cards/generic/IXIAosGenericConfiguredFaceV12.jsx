...
function faceConfig(object = {}, faceNumber = 2) {
...
function fieldDefinitionsForFace(object = {}, faceNumber = 2) {
...
function formatValue(value, type = "", currency = "USD") {
  ...
  if (["money", "currency"].includes(normalizedType) && Number.isFinite(number)) {
  ...
  if (["number", "integer"].includes(normalizedType) && Number.isFinite(number)) {
...
function getConfiguredValue(object = {}, item = {}, runtimeData = {}) {
  const source = clean(item?.source || "fields").toLowerCase();
  const key = clean(item?.key || item?.fieldId || item?.field || item?.id);
  if (!key) return null;

  if (source === "object") return object?.[key] ?? null;
  if (source === "metadata") return object?.metadata?.[key] ?? null;
  if (source === "runtime" || source === "projection") return runtimeData?.[key] ?? null;
  return getObjectFields(object)?.[key] ?? null;
}

function normalizedFieldItem(object, item = {}) {
  const fieldId = clean(item?.fieldId || item?.field || item?.key || item?.id);
  ...
  return {
    ...
    fieldType: clean(item?.type || item?.fieldType || definition?.fieldType || "text")
  };
}

function sectionItems(object, section = {}, fallbackDefinitions = []) {
  const configured = asArray(section?.fields || section?.items || section?.metrics);
  if (configured.length) return configured.map(item => normalizedFieldItem(object, item));
  return fallbackDefinitions.map(definition => normalizedFieldItem(object, definition));
}

function Editor({ object, definitions, saving, onCancel, onSave }) {
  ...
  useEffect(() => {
    ...
    definitions.forEach(definition => {
      ...
      next[definition.fieldId] = Array.isArray(value) ? value.join(", ") : String(value ?? "");
    });
    setDraft(next);
  }, [object, definitions]);
  ...
  async function save() {
    ...
    definitions.forEach(definition => {
      ...
      if (["number", "integer", "money", "currency"].includes(type)) {
        ...
        fields[definition.fieldId] = Number.isFinite(value) ? value : null;
...
export default function IXIAosGenericConfiguredFaceV12({
  ...
  const config = useMemo(() => faceConfig(runtimeObject, faceNumber), [runtimeObject, faceNumber]);
  const declaredDefinitions = useMemo(() => fieldDefinitionsForFace(runtimeObject, faceNumber), [runtimeObject, faceNumber]);
  ...
  const resolvedSections = sections.length
    ? sections
    : declaredDefinitions.length
      ? [{ title: clean(config?.defaultSectionTitle) || "DETAILS", layout: "rows", fields: declaredDefinitions }]
      : [];

  const editableDefinitions = declaredDefinitions.filter(definition => definition.editable !== false);
  ...
  return (
    <article className={`ixi-generic-face-v12 skin-${skinId}`} data-face-number={faceNumber}>
      <header className="gfv12-head">
        <div className="gfv12-ident"><span>{eyebrow}</span><strong>{getObjectDisplayName(runtimeObject)}</strong></div>
        {!editing ? (
      ...
      <main className="gfv12-scroll">
        ...
        {resolvedSections.map((section, sectionIndex) => {
          const items = sectionItems(runtimeObject, section, sectionIndex === 0 ? declaredDefinitions : []);
              ...
              <section className="gfv12-section" key={`${sectionTitle}-${sectionIndex}`}>
                ...
                <div className="gfv12-relations">
                  {relationships.map(relationship => (
                    <button type="button" key={relationship.id}><span><small>{relationship.label}</small><strong>{relationship.value}</strong>{relationship.secondary ? <em>{relationship.secondary}</em> : null}</span><b>›</b></button>
                  ))}
            ...
            <section className="gfv12-section" key={`${sectionTitle}-${sectionIndex}`}>
              ...
              <div className={layout === "grid" || layout === "metrics" ? "gfv12-grid" : "gfv12-rows"}>
                {items.map((item, index) => {
                  const value = getConfiguredValue(runtimeObject, item, runtimeData);
                  return (
                    <div className="gfv12-value" key={`${item.fieldId || item.key || index}-${index}`}>
                      <small>{item.label}</small>
                      <strong>{formatValue(value, item.fieldType, currency)}</strong>
        ...
        {config?.showRelationships === true && !resolvedSections.some(section => clean(section?.layout || section?.type).toLowerCase() === "relationships") ? (
          <section className="gfv12-section"><h3>{clean(config?.relationshipsTitle) || "RELATIONSHIPS"}</h3><div className="gfv12-relations">{relationships.map(relationship => <button type="button" key={relationship.id}><span><small>{relationship.label}</small><strong>{relationship.value}</strong></span><b>›</b></button>)}</div></section>
        ) : null}

        {!resolvedSections.length ? <div className="gfv12-unconfigured"><b>F{faceNumber}</b><strong>FACE NOT CONFIGURED</strong><span>This layout renders only fields explicitly assigned to this face.</span></div> : null}
      </main>
      ...
      {editing ? <Editor object={runtimeObject} definitions={editableDefinitions} saving={saving} onCancel={() => setEditing(false)} onSave={save}/> : null}
