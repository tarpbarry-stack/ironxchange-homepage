...
export default function IXIAosGenericMediaDominant009({
  ...
  const primaryFields = populatedFields.slice(0, 3);
      ...
      <main className="c009-body">
        ...
        <section className="c009-primary-grid">
          {primaryFields.map(({ definition, value }) => (
            <div className="c009-field-block" key={definition.fieldId}><span>{definition.label}</span><strong>{value}</strong></div>
          ))}
          {!primaryFields.length ? <div className="c009-field-block c009-field-empty"><span>DETAILS</span><strong>—</strong></div> : null}
        </section>
