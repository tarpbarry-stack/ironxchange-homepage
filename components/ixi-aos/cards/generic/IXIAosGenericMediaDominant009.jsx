...
function editableDefinitionsFor(object = {}) {
  ...
  const used = new Set(output.map(item => item.fieldId));
  let index = 0;

  while (output.length < MIN_FIELDS) {
      ...
      <div className="c009-editor-scroll">
        ...
        <section>
          ...
          {definitions.map((definition, index) => (
            <div className="c009-editor-row" key={definition.fieldId}>
              <input
...
export default function IXIAosGenericMediaDominant009({
  ...
  async function addPrimaryPhoto(event) {
    ...
    await save({
      ...runtimeObject,
      media: [photo, ...asArray(runtimeObject?.media).slice(1)]
    });
    event.target.value = "";
  }
      ...
      <main className="c009-body">
        <section className="c009-media">
          {image ? (
            ...
            <div className="c009-media-empty"><b>IXI</b><span>PRIMARY MEDIA</span></div>
          )}
        ...
        <section className="c009-detail-strip">
          {secondaryFields.map(({ definition, value }) => (
            <div className="c009-detail" key={definition.fieldId}>
              <span>{definition.label}</span><strong>{value}</strong>
            </div>
          ))}
        </section>
