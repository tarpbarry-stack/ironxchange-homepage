...
const COPY = {
  en: {
    ...
    inventoryPolicy:
      "Inventory usage records the durable stock decrement with this material event.",
    poPolicy:
      "PO usage consumes from received supply and links back to the PO/receiving record. It does not create another purchase.",
    supplyPolicy:
      "Existing supply records physical consumption only. No new Expense or Purchase is created.",
    footer:
      "Material Usage attributes physical cost to this AOS context. It is not a second cash-spend event.",
    required:
      "Complete the required material, quantity, cost, date and source information.",
  ...
  es: {
    ...
    inventoryPolicy:
      "El uso de inventario registra la reducción durable de existencias con este evento de material.",
    poPolicy:
      "El uso desde OC consume suministro recibido y se vincula a la OC/recepción. No crea otra compra.",
    supplyPolicy:
      "Suministro existente registra solo consumo físico. No crea otro Gasto ni Compra.",
    footer:
      "Uso de Material atribuye costo físico a este contexto AOS. No es un segundo evento de gasto de efectivo.",
...
function normalizeInventoryItems(object = {}, inventoryItems = []) {
  const candidates =
    Array.isArray(inventoryItems) && inventoryItems.length
      ? inventoryItems
  ...
  return candidates
    .map((item, index) => ({
      id:
        clean(item.inventoryItemId || item.id || item.objectId) ||
        `INV-${index + 1}`,
      passportId: clean(item.inventoryPassportId || item.passportId),
      description: clean(
        item.description || item.name || item.label || item.title,
      ),
      ...
      unitCost: Math.max(
        0,
        num(item.unitCost ?? item.averageCost ?? item.cost),
      ),
      sourceLocationId: clean(item.locationId || item.sourceLocationId),
      sourceLocationLabel: clean(
        item.locationLabel || item.sourceLocationLabel || item.location,
      ),
    }))
...
function normalizePoLines(object = {}, purchaseOrderLines = []) {
  const candidates =
    Array.isArray(purchaseOrderLines) && purchaseOrderLines.length
      ? purchaseOrderLines
  ...
  return candidates
    .map((line, index) => {
      const received = Math.max(
        0,
        num(line.receivedQuantity ?? line.received ?? line.quantityReceived),
      );
      const consumed = Math.max(
        0,
        num(line.consumedQuantity ?? line.usedQuantity ?? line.quantityUsed),
      );
      const explicitAvailable =
        line.availableQuantity ?? line.remainingQuantity;
      return {
        id:
          clean(line.purchaseOrderLineId || line.lineId || line.id) ||
          `POL-${index + 1}`,
        purchaseOrderId: clean(line.purchaseOrderId || line.poId),
        purchaseOrderNumber: clean(
          line.purchaseOrderNumber || line.poNumber || line.po,
        ),
...
export default function IXIMaterialStandaloneApp({
  ...
  const [source, setSource] = useState(() =>
    normalizeInventoryItems(object, inventoryItems).length
      ? "inventory"
      : "manual",
  );
    ...
    () =>
      financialRecords.reduce(
        (totals, raw) => {
          ...
          if (clean(document?.documentType).toLowerCase() !== "material-usage")
            return totals;
          const material =
            document?.materialUsage?.material || document?.material || {};
          const quantityUsed = Math.max(0, num(material.quantity));
          const inventoryKey = clean(
            material.inventoryItemId || material.inventoryPassportId,
          );
          const poKey = clean(material.purchaseOrderLineId);
          if (inventoryKey)
            totals.inventory[inventoryKey] =
              (totals.inventory[inventoryKey] || 0) + quantityUsed;
          if (poKey)
            totals.purchaseOrder[poKey] =
              (totals.purchaseOrder[poKey] || 0) + quantityUsed;
          return totals;
        },
  ...
  const inventory = useMemo(
    () =>
      normalizeInventoryItems(object, inventoryItems).map((item) => ({
        ...item,
        available: Math.max(
          0,
          item.available -
            (consumption.inventory[item.id] ||
              consumption.inventory[item.passportId] ||
              0),
        ),
      })),
    [object, inventoryItems, consumption],
  );
  const poLines = useMemo(
    () =>
      normalizePoLines(object, purchaseOrderLines).map((line) => ({
        ...line,
        available: Math.max(
          0,
          line.available - (consumption.purchaseOrder[line.id] || 0),
        ),
      })),
    [object, purchaseOrderLines, consumption],
  );
  const primary = context.primary || {};
  const actor = context.actor || {};
  const location = context.location || {};
  const workOrder = context.activeWorkOrder || {};
  const workNumber = clean(
    workOrder?.identity?.number ||
      workOrder?.workOrderNumber ||
      workOrder?.number,
  );
  const extendedCost =
    Math.round(Math.max(0, num(quantity)) * Math.max(0, num(unitCost)) * 100) /
    100;
  ...
  async function save() {
    if (saving) return;
    const inventoryItem =
      inventory.find((entry) => entry.id === inventoryId) || {};
    ...
    const input = {
      ...
      sourceLocationLabel,
      inventoryItemId:
        source === "inventory" ? clean(inventoryItem.id || inventoryId) : "",
      inventoryPassportId:
        source === "inventory" ? clean(inventoryItem.passportId) : "",
      purchaseOrderId:
        source === "purchase-order" ? clean(poLine.purchaseOrderId) : "",
      purchaseOrderNumber:
        source === "purchase-order"
          ? clean(poLine.purchaseOrderNumber || purchaseOrderNumber)
          : "",
      purchaseOrderLineId:
        source === "purchase-order" ? clean(poLine.id || poLineId) : "",
      receivingRecordId:
        source === "purchase-order" ? clean(poLine.receivingRecordId) : "",
      dateUsed,
      condition: "good",
      referenceNotes:
        source === "purchase-order" ? clean(poLine.receivingRecordId) : "",
    ...
    try {
      const persisted = await createIXIMaterialUsage({
        object: {
          ...
          label: clean(
            object.label || object.displayName || object.name || primary.label,
          ),
        },
        context,
        workOrder,
    ...
    return (
      <div className="mat-v13">
        <div className="ms-saved">
          ...
          <h3>{t.saved}</h3>
          <strong>
            {clean(saved.identity?.materialUsageId) || "MAT-#####"}
          </strong>
          <p>
            {saved.material?.description} · {saved.material?.quantity}{" "}
            {saved.material?.unit} · $
            {num(saved.material?.extendedCost).toFixed(2)}
          </p>
          <p>
            {clean(primary.label)} ·{" "}
            {clean(saved.material?.source).replaceAll("-", " ").toUpperCase()}
          </p>
  ...
  return (
    <div className="mat-v13">
      ...
      <div className="ms-context">
        ...
        <div>
          <b>{clean(location.label) || "—"}</b>
          <small>{t.location}</small>
        </div>
        <div>
          <b>{clean(actor.displayName || actor.name || actor.label) || "—"}</b>
      ...
      <div className="ms-source">
        <button
          className={source === "inventory" ? "on" : ""}
          onClick={() => resetSource("inventory")}
        >
          <span>▣</span>
          {t.inventory}
        </button>
        <button
          className={source === "purchase-order" ? "on" : ""}
          onClick={() => resetSource("purchase-order")}
        >
          <span>▤</span>
          {t.po}
        </button>
        <button
          className={source === "manual" ? "on" : ""}
          onClick={() => resetSource("manual")}
        >
          <span>◇</span>
          {t.supply}
        </button>
      ...
      {source === "inventory" && inventory.length ? (
        <>
          ...
          <div className={`ms-field ${errors.description ? "bad" : ""}`}>
            <select
              value={inventoryId}
              onChange={(event) => chooseInventory(event.target.value)}
            >
              <option value="">{t.noInventory}</option>
              {inventory.map((item) => (
        ...
        <>
          <label>
            {t.receivedItem} <em>*</em>
          </label>
          <div className={`ms-field ${errors.purchaseOrder ? "bad" : ""}`}>
            <select
              value={poLineId}
              onChange={(event) => choosePoLine(event.target.value)}
            >
        ...
        <>
          ...
          <div className={`ms-field ${errors.description ? "bad" : ""}`}>
            <input
              ...
              placeholder="CAT hydraulic filter"
            />
          </div>
        </>
      )}
      ...
      <div className="ms-two">
        ...
        <div>
          ...
          <div className={`ms-field ${errors.dateUsed ? "bad" : ""}`}>
            <input
              type="date"
              value={dateUsed}
      ...
      <div className="ms-three">
        <div>
          ...
          <div
            className={`ms-field ${errors.quantity || quantityTooHigh ? "bad" : ""}`}
          >
            <input
              inputMode="decimal"
              value={quantity}
      ...
      {["inventory", "purchase-order"].includes(source) ? (
        <div className={`ms-availability ${quantityTooHigh ? "warning" : ""}`}>
          <span>{t.available}</span>
          <b>
            {availableQuantity || 0} {unit}
          </b>
        </div>
      ) : null}
      ...
      {source === "inventory" ? (
        <>
          <label>
            {t.sourceLocation} <em>*</em>
          </label>
          <div className={`ms-field ${errors.sourceLocation ? "bad" : ""}`}>
            <input
              value={sourceLocationLabel}
              onChange={(event) => setSourceLocationLabel(event.target.value)}
              placeholder={clean(location.label) || "Shop Inventory"}
            />
          </div>
        </>
      ) : null}
      ...
      {Object.keys(errors).length || quantityTooHigh ? (
        <div className="ms-errors">
          {quantityTooHigh ? t.exceeds : t.required}
        </div>
      ) : null}
      ...
      <div className="ms-actions">
        ...
        <button className="use" onClick={save} disabled={saving}>
          {saving ? t.saving : t.use}
        </button>
      </div>
