...
export default function IXIPurchaseApp({
  ...
  const workOrderNumber = clean(workOrder.identity?.number || workOrder.workOrderNumber || workOrder.number) || "WORK ORDER";
  ...
  }), [requestType, vendorLabel, neededByDate, priority, items, shipping, workOrderNumber, costCode, notes, attachment]);
  ...
  async function save() {
    ...
    const nextDraft = createIXIPurchaseDraft({ context, workOrder, input });
    ...
    try {
      ...
      if (clean(primary.passportId) && workOrderId) {
        persisted = await createIXIPurchase({
          ...
          workOrder,
          input,
          metadata: { source: "ixi-transact-work-order-purchase" }
        });
      }
  ...
  return (
    <div className="tx-purchase">
      <div className="po-lang" aria-label="Language">
        <button type="button" className={lang === "en" ? "on" : ""} onClick={() => setLang("en")} disabled={saving}>ENG</button>
        <span>/</span>
        <button type="button" className={lang === "es" ? "on" : ""} onClick={() => setLang("es")} disabled={saving}>ESP</button>
      </div>
      ...
      <div className="po-modes">
        ...
        <button type="button" className={requestType === "purchase-order" ? "on" : ""} onClick={() => setRequestType("purchase-order")} disabled={saving}>{t.po}</button>
      </div>
      ...
      <div className={`po-lines ${errors.items || errors.itemLine ? "invalid" : ""}`}>
        ...
        {items.map((item, index) => (
          <div className="po-line" key={item.lineId || index}>
            <input aria-label={t.description} value={item.description} onChange={event => patchItem(index, "description", event.target.value)} placeholder={t.description} disabled={saving} />
            <input aria-label={t.qty} inputMode="decimal" value={item.quantity} onChange={event => patchItem(index, "quantity", event.target.value)} disabled={saving} />
      ...
      <div className="po-two">
        ...
        <div>
          <label>{t.total}</label>
          <div className="po-field"><span aria-hidden="true">▤</span><input className="po-total" readOnly value={`$${draft.purchase.estimatedTotal.toFixed(2)}`} /></div>
        </div>
      </div>
      ...
      <div className="po-two">
        <div>
          ...
          <div className="po-field locked-field"><input readOnly value={workOrderNumber} /></div>
        </div>
        <div>
          <label>{t.cost}</label>
      ...
      <div className="po-attach">
        <label className="po-file-button">
          ...
          <input type="file" accept="application/pdf,image/jpeg,image/png" onChange={event => setAttachment(event.target.files?.[0] || null)} disabled={saving} />
        </label>
      </div>

      {Object.keys(errors).length ? <div className="po-errors" role="alert">{t.required}</div> : null}
      {saveError ? <div className="po-errors po-save-error" role="alert">{saveError === t.saveFailed ? saveError : `${t.saveFailed} (${saveError})`}</div> : null}
