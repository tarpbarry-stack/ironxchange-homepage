...
export async function createIXIPurchase({
  ...
  const draft = createIXIPurchaseDraft({
    context,
    workOrder,
    input
  });
  ...
  for (const [candidate, role] of [
    ...
    const reference = createIXIAosFinancialObjectReference({
      object: candidate,
      role
    });
  ...
  if (vendorReference) {
    references.push(vendorReference);
  }
  ...
  if (workOrderReference) {
    references.push(workOrderReference);
  }
  ...
  const response = await createIXIAosObjectFinancialDocument({
    object,
    documentType:
      purchase.requestType === "purchase-order"
        ? "purchase-order"
    ...
    input: {
      ...
      description: `${purchase.requestType}: ${purchase.items
