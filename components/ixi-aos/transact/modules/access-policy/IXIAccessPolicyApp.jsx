...
const CAPABILITY_GROUPS = Object.freeze([
  {
    label: "AOS",
    capabilities: ["aos.discover", "aos.view", "aos.edit", "aos.move", "aos.delete"]
  },
  {
    label: "WORK",
    capabilities: ["work-order.view", "work-order.time.create", "work-order.material.create"]
  },
  {
    label: "TRAN$ACT",
    capabilities: ["transact.gl.view", "transact.treasury.view"]
  },
  {
    label: "ADMIN",
    capabilities: ["authority.manage", "identity.invite"]
  }
]);
...
async function jsonRequest(url, options = {}) {
  ...
  if (!response.ok || payload?.ok === false) {
    const error = new Error(
      ...
      `Authority request failed (${response.status}).`
    );
...
function subjectLabel(rule = {}) {
  const type = clean(rule?.subject?.type).toUpperCase();
  const id = clean(rule?.subject?.id);
  return id || type || "SUBJECT";
}

function scopeLabel(rule = {}) {
  return clean(rule?.scope?.type || "target").replaceAll("-", " ").toUpperCase();
}
...
export default function IXIAccessPolicyApp({
  ...
  const grants = unique([
    ...(access?.principal?.directGrants || []),
    ...(access?.capabilities?.directGrants || [])
  ]);
  const denies = unique([
    ...(access?.principal?.directDenies || []),
    ...(access?.capabilities?.directDenies || [])
  ]);
  ...
  async function loadAuthority() {
    ...
    try {
      ...
      if (loadError?.status === 401 || loadError?.code === "IXI_AUTHENTICATION_REQUIRED" || loadError?.code === "IXI_ACCESS_TOKEN_INVALID") {
  ...
  async function removeRule(rule) {
    ...
    await writePolicy(rules.filter(item => item.ruleId !== rule.ruleId));
  }
    ...
    <div className="ap-shell">
      <header className="ap-header">
        ...
        <div className="title">
          ...
          <small>{draftObject ? "DRAFT · NOT YET PASSPORT-ADDRESSABLE" : (passportId || "LEGACY OBJECT · PASSPORT REQUIRED")}</small>
        </div>
        <div className="mode">
      ...
      <main>
        ...
        {authRequired && !loading ? (
          <form className="login" onSubmit={signIn}>
            ...
            <small>Cognito credentials are exchanged server-side. The access token remains HttpOnly and is never exposed to this card.</small>
        ...
        {!loading && !authRequired ? (
          <>
            ...
            {draftObject ? (
              <div className="message passport-state">DRAFT OBJECTS DO NOT RECEIVE PASSPORTS OR AUTHORITY POLICY. SAVE THE OBJECT FIRST; THE PERMANENT OBJECT + PASSPORT ARE CREATED TOGETHER.</div>
            ) : null}

            {!draftObject && !passportId ? (
              <div className="message error">THIS IS A PRE-PROVISIONING / LEGACY OBJECT WITHOUT IXI PASSPORT IDENTITY. POLICY WRITES ARE FAIL-CLOSED.</div>
            ) : null}
              ...
              <>
                <section>
                  <div className="section-title">
                    ...
                    {canManage ? (
                      <button type="button" disabled={saving} onClick={() => writePolicy(rules, { ...inheritance, propagateToChildren: !inheritance.propagateToChildren })}>
                        {inheritance.propagateToChildren ? "DESCENDANTS ON" : "TARGET ONLY"}
                      </button>
                    ) : null}
                  </div>
                  ...
                  {rules.map(rule => (
                    <article key={rule.ruleId || `${rule.effect}-${rule.subject?.type}-${rule.subject?.id}`}>
                      ...
                      <div className="rule-copy">
                        <strong>{subjectLabel(rule)}</strong>
                        <span>{(rule.capabilities || []).join(" · ")}</span>
                        <small>{scopeLabel(rule)}</small>
                      </div>
                      {canManage ? <button type="button" className="remove" disabled={saving} onClick={() => removeRule(rule)}>×</button> : null}
                    </article>
                  ))}
                </section>
                  ...
                  <section className="builder">
                    ...
                    <div className="cap-groups">
                      {CAPABILITY_GROUPS.map(group => (
                        <div key={group.label} className="cap-group">
      ...
      <style jsx>{`
        .ap-shell,.ap-shell *{box-sizing:border-box}.ap-shell{width:298px;height:471px;overflow:hidden;border:1px solid rgba(255,196,0,.22);border-radius:12px;background:linear-gradient(180deg,rgba(255,196,0,.04),transparent 27%),#090b0b;color:#eef0ee;font-family:Arial,sans-serif;box-shadow:0 18px 38px rgba(0,0,0,.48)}
        .ap-header{height:56px;display:flex;align-items:center;gap:7px;padding:7px 8px;border-bottom:1px solid rgba(255,255,255,.08);background:rgba(255,255,255,.012)}.back{width:23px;height:34px;border:1px solid rgba(255,255,255,.1);border-radius:5px;background:#111414;color:#ffc400;font-size:20px;cursor:pointer}.title{min-width:0;flex:1}.title span{display:block;color:#ffc400;font-size:6px;font-weight:950;letter-spacing:.11em}.title strong{display:block;margin-top:2px;font-size:13px;font-weight:950;letter-spacing:-.02em}.title small{display:block;margin-top:2px;color:#7b817e;font-size:5px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.mode{text-align:right}.mode b{display:block;color:#ffc400;font-size:6px}.mode small{font-size:5px;color:#777d7a}
        main{height:415px;overflow-y:auto;padding:8px;scrollbar-width:thin}.message,.empty{padding:7px;border:1px solid rgba(255,255,255,.08);border-radius:5px;background:#101313;color:#929895;font-size:6px;font-weight:850;line-height:1.45;margin-bottom:6px}.error{border-color:rgba(255,86,86,.35);color:#ff9898}.notice{border-color:rgba(255,196,0,.28);color:#ffc400}.passport-state{border-color:rgba(88,178,255,.24);color:#a8d6ff}.summary{display:grid;grid-template-columns:1.25fr .75fr .75fr;gap:4px;margin-bottom:8px}.summary div{min-width:0;padding:6px;border:1px solid rgba(255,255,255,.07);border-radius:4px;background:#0f1212}.summary span,label span{display:block;color:#727875;font-size:5px;font-weight:950;letter-spacing:.06em}.summary b{display:block;margin-top:3px;font-size:6px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.section-title{display:flex;align-items:center;justify-content:space-between;margin:8px 1px 4px}.section-title span{color:#858b88;font-size:5.5px;font-weight:950;letter-spacing:.11em}.section-title button{padding:4px 6px;border:1px solid rgba(255,196,0,.2);border-radius:4px;background:#141616;color:#ffc400;font-size:5px;font-weight:900;cursor:pointer}
        article{display:grid;grid-template-columns:42px 1fr 20px;gap:6px;align-items:center;padding:6px;margin-bottom:4px;border:1px solid rgba(255,255,255,.07);border-radius:5px;background:#0f1212}.effect{padding:5px 3px;border-radius:3px;text-align:center;font-size:5.5px;background:rgba(66,211,146,.1);color:#6ce3ad}.effect.deny{background:rgba(255,78,78,.1);color:#ff8a8a}.rule-copy{min-width:0}.rule-copy strong{display:block;font-size:6.5px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.rule-copy span{display:block;margin-top:2px;color:#9da3a0;font-size:5px;line-height:1.3}.rule-copy small{display:block;margin-top:2px;color:#666d69;font-size:4.7px}.remove{width:20px;height:22px;border:1px solid rgba(255,76,76,.18);border-radius:4px;background:#161111;color:#ff8b8b;cursor:pointer}
        .builder{padding-bottom:10px}.two{display:grid;grid-template-columns:1fr 1fr;gap:5px}label{display:block;margin-bottom:5px}input,select{width:100%;height:27px;margin-top:3px;padding:0 7px;border:1px solid rgba(255,255,255,.09);border-radius:4px;background:#111414;color:#e9ecea;font-size:6.5px;outline:none}input:focus,select:focus{border-color:rgba(255,196,0,.38)}.cap-groups{display:grid;gap:4px;margin:5px 0}.cap-group{padding:5px;border:1px solid rgba(255,255,255,.06);border-radius:4px;background:#0d1010}.cap-group>span{display:block;margin-bottom:4px;color:#646b67;font-size:4.7px;font-weight:950}.cap-group>div{display:flex;flex-wrap:wrap;gap:3px}.cap-group button{padding:4px 5px;border:1px solid rgba(255,255,255,.08);border-radius:3px;background:#141717;color:#777e7a;font-size:4.8px;font-weight:800;cursor:pointer}.cap-group button.active{border-color:rgba(255,196,0,.4);background:rgba(255,196,0,.1);color:#ffd95c}.save{width:100%;height:29px;border:1px solid rgba(255,196,0,.42);border-radius:5px;background:linear-gradient(180deg,#2a2410,#15130d);color:#ffc400;font-size:6px;font-weight:950;letter-spacing:.08em;cursor:pointer}.save:disabled,.section-title button:disabled{opacity:.45;cursor:default}
        .login{margin-top:4px;padding:10px;border:1px solid rgba(255,196,0,.18);border-radius:6px;background:#0e1111}.login .eyebrow{color:#ffc400;font-size:5px;font-weight:950;letter-spacing:.12em}.login strong{display:block;margin-top:4px;font-size:11px}.login small{display:block;margin:5px 0 8px;color:#858b88;font-size:5.8px;line-height:1.45}.login button{width:100%;height:29px;margin-top:3px;border:1px solid rgba(255,196,0,.38);border-radius:4px;background:#19170e;color:#ffc400;font-size:6px;font-weight:950}
      `}</style>
    </div>
  );
}
