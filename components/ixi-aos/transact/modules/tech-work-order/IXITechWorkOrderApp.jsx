...
import { createIXITechWorkOrderDraft, normalizeIXITechWorkOrder } from "./IXITechWorkOrderContract";
import { applyIXITechWorkOrderAction, getIXITechWorkOrderActuals } from "./IXITechWorkOrderEngine";
...
function addReference(record, key, id, financialKey = "", amount = 0) {
  ...
  return {
    ...record,
    references: { ...(record.references || {}), [key]: [...current, referenceId] },
    financial: financialKey
      ? { ...(record.financial || {}), [financialKey]: number(record?.financial?.[financialKey]) + number(amount) }
      : record.financial
  };
}
...
function addDocuments(record, attachments = [], sourceType = "general", sourceId = "", issuer = "", actorName = "") {
  ...
  return { ...record, documentProjection: next };
}
...
export default function IXITechWorkOrderApp({
  ...
  const [record, setRecord] = useState(initialTechWorkOrder ? normalizeIXITechWorkOrder(initialTechWorkOrder) : null);
  ...
  const actuals = useMemo(() => getIXITechWorkOrderActuals(record || {}), [record]);
  ...
  }, [context.actor?.passportId, context.actor?.employeeId, context.actor?.userId]);
  ...
  async function create() {
    ...
    try {
      ...
      setError(clean(err?.message) || "TECHWO CREATE FAILED");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function lifecycle(action, payload = {}) {
    ...
    setError("");
    try {
      const next = applyIXITechWorkOrderAction({ record, action, actor: context.actor || {}, payload });
      await commit(next, { action, payload });
      setNotice(action === "pause" ? t.workPaused : action === "resume" ? t.workResumed : action === "complete" ? t.completeState : action.toUpperCase());
    } catch (err) {
  ...
  async function savePurchase(draft, input, response) {
    ...
    let next = addReference(record, isOrder ? "purchaseOrderIds" : "purchaseRequestIds", id, isOrder ? "committed" : "requested", amount);
  ...
  async function saveNote(note) {
    ...
    const stored = { ...note, identity: { ...(note.identity || {}), noteId: id } };
    const next = {
      ...record,
      references: { ...(record.references || {}), noteIds: unique([...(record.references?.noteIds || []), id]) },
      noteProjection: [...(record.noteProjection || []), stored],
      activityProjection: [...(record.activityProjection || []), {
  ...
  async function savePhoto(photo) {
    ...
    const stored = { ...photo, identity: { ...(photo.identity || {}), photoId: id } };
    ...
    const next = {
      ...
      references: {
        ...
        attachmentIds: unique([...(record.references?.attachmentIds || []), ...docs.map(item => item.documentId)])
      },
      photoProjection: [...(record.photoProjection || []), stored],
      documentProjection: [...(record.documentProjection || []), ...docs.filter(doc => !(record.documentProjection || []).some(existing => existing.documentId === doc.documentId))],
      activityProjection: [...(record.activityProjection || []), {
        activityId: `TECHACT-${Date.now()}`,
        type: "photo-added",
        actorLabel: actorName,
  ...
  async function addGeneralDocument(input) {
    ...
    const next = {
      ...record,
      documentProjection: existing.some(item => clean(item.documentId) === id) ? existing : [...existing, document],
      references: { ...(record.references || {}), attachmentIds: unique([...(record.references?.attachmentIds || []), id]) }
    };
  ...
  async function recordTimerDelta(session, action) {
    ...
    let draft = { identity: { timeEntryId: `TIME-${Date.now()}` }, time: { hours } };
    let response = null;
    if (clean(context.primary?.passportId) && clean(record?.identity?.workOrderId)) {
      const persisted = await createIXITimeEntry({
        object: { passportId: context.primary.passportId, objectType: context.primary.objectType, label: context.primary.label },
        context,
        workOrder: record,
        input,
        metadata: { source: "ixi-transact-techwo-live-timer", techWorkOrderId: record.identity.techWorkOrderId, timerSessionId: session.sessionId, action }
      });
  ...
  async function resumeWork() {
    ...
    if (session?.status === "paused") session = resumeIXITimeSession(context);
  ...
  if (record && submodule === "expense") return <IXIExpenseApp context={context} workOrder={record} language={lang} onLanguageChange={setLang} onCancel={() => setSubmodule("")} onSave={saveExpense} />;
  if (record && submodule === "material") return <IXIMaterialApp context={context} workOrder={record} language={lang} onLanguageChange={setLang} onCancel={() => setSubmodule("")} onSave={saveMaterial} />;
  if (record && submodule === "time") return <IXITimeEntryApp context={context} workOrder={record} language={lang} onLanguageChange={setLang} onCancel={() => setSubmodule("")} onSave={saveTime} onLiveStart={handleLiveStart} />;
  if (record && submodule === "service") return <IXIServiceApp context={context} workOrder={record} language={lang} onLanguageChange={setLang} onCancel={() => setSubmodule("")} onSave={saveService} />;
  if (record && submodule === "purchase") return <IXIPurchaseApp context={context} workOrder={record} language={lang} onLanguageChange={setLang} onCancel={() => setSubmodule("")} onSave={savePurchase} />;
  if (record && submodule === "note") return <IXINoteApp context={context} workOrder={record} onCancel={() => setSubmodule("")} onSave={saveNote} />;
  if (record && submodule === "photo") return <IXIPhotoApp context={context} workOrder={record} onCancel={() => setSubmodule("")} onSave={savePhoto} />;
  if (record && submodule === "documents") return <IXIWorkOrderDocumentsApp context={context} workOrder={record} documents={record.documentProjection || []} language={lang} onLanguageChange={setLang} onBack={() => setSubmodule("")} onDocumentAction={(actionId, row) => onRecordChange?.(record, { action: `document-${actionId}`, document: row }, context)} onAddGeneralDocument={addGeneralDocument} />;
    ...
    return (
      <div className="wo-app wo-v13 techwo-v13">
        ...
        <div className="wo-title">
          ...
          <div><span className="techwo-titlemark">TECHWO# XXXXXX</span><strong>{t.new}</strong><small>{t.sub}</small></div>
        </div>
        ...
        <div className="techwo-tech-fields">
          <select value={environment} onChange={event => setEnvironment(event.target.value)} aria-label={t.environment}>
            <option value="production">PRODUCTION</option><option value="test">TEST</option><option value="development">DEVELOPMENT</option><option value="field">FIELD</option>
          </select>
        ...
        <IXIWorkOrderStyles /><IXITechWorkOrderStyles />
      </div>
    );
  }
    ...
    <div className={`wo-app wo-v13 wo-work techwo-v13 ${isPaused ? "paused" : ""}`}>
      ...
      <div className="wo-work-identity">
        ...
        <div className="wo-work-copy">
          <div className="wo-number-row"><strong>{numberLabel}</strong><span>{isComplete ? t.completeState : isPaused ? t.paused : status === "waiting" ? t.waiting : t.inProgress}</span></div>
          <h3>{objectLabel}</h3>
      ...
      <div className="wo-scroll">
        {tab === "work" ? <>
          ...
          <div className="wo-status-row"><strong>{status.toUpperCase()}</strong><span>{clean(record.people?.requestedBy?.label) || actorName}</span></div>
          ...
          <div className="wo-duo">
            <button onClick={() => setSubmodule("note")}><MicIcon size={15} /><b>{t.note}</b></button>
            <button onClick={() => setSubmodule("photo")}><CameraIcon size={15} /><b>{t.photoAction}</b></button>
          </div>

          {!isComplete ? <div className="wo-bottom-actions">
            <button onClick={isPaused ? resumeWork : pauseWork}>{isPaused ? <RefreshIcon size={15} /> : <PauseIcon size={15} />}<span>{isPaused ? t.resume : t.pause}</span></button>
            <button onClick={() => setCompletionText(clean(record.technology?.resolution))}><span>{t.complete}</span></button>
          </div> : null}

          {!isComplete && completionText !== "" ? <div className="techwo-complete-panel">
            ...
            <button disabled={busy} onClick={() => lifecycle("complete", { disposition: "fully-functioning", resolution: completionText, workPerformed: completionText, validation: "Completed by assigned technician", finalImpact: "normal" })}>{t.finish}</button>
          </div> : null}

          {isComplete ? <button className="techwo-reopen" disabled={busy} onClick={() => lifecycle("reopen", { reason: "Work requires additional attention" })}>{t.reopen}</button> : null}
        </> : null}
        ...
        {tab === "activity" ? <div className="techwo-activity">
          {activity.length ? [...activity].reverse().map(item => <div className="techwo-activity-row" key={item.activityId}><i /><span><strong>{clean(item.type).replaceAll("-", " ").toUpperCase()}</strong><small>{clean(item.actorLabel)}{item.note ? ` · ${item.note}` : ""}</small></span><time>{item.occurredAt ? new Date(item.occurredAt).toLocaleString(lang === "es" ? "es-MX" : "en-US") : ""}</time></div>) : <div className="wo-description">{t.noActivity}</div>}
        </div> : null}
