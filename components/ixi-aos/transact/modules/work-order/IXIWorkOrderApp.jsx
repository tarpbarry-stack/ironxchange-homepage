...
export default function IXIWorkOrderApp({context={},initialWorkOrder=null,onBack=null,onCreate=null,onAction=null}){
  ...
  return <div className="wo-app">
    ...
    <section><div className="section-head"><b>WORK</b><em>{title(workOrder.work?.priority)}</em></div><p>{workOrder.work?.description}</p><div className="wo-action-row"><button type="button" onClick={()=>action("note")}>+ NOTE</button><button type="button" onClick={()=>action("photo")}>+ PHOTO</button></div></section>
    <section><div className="metric"><span>TIME</span><strong>{counts.time}</strong><button type="button" onClick={()=>action("time")}>+ TIME</button></div><div className="metric"><span>MATERIAL</span><strong>${actuals.materialActual.toLocaleString()}</strong><button type="button" onClick={()=>action("material")}>+ MATERIAL</button></div><div className="metric"><span>SERVICE</span><strong>${actuals.serviceActual.toLocaleString()}</strong><button type="button" onClick={()=>action("service")}>+ SERVICE</button></div><div className="metric"><span>EXPENSE</span><strong>${actuals.otherActual.toLocaleString()}</strong><button type="button" onClick={()=>action("expense")}>+ EXPENSE</button></div><div className="metric"><span>PURCHASING</span><strong>{counts.purchaseOrders}</strong><button type="button" onClick={()=>action("purchase-order")}>+ PO</button></div></section>
