...
export async function createIXIWorkOrder({object={},context={},input={},commandId="",idempotencyKey="",metadata={},apiBaseUrl="",headers={},signal}={}){
  ...
  const response=await createIXIAosWorkOrder({object,input:{currency:"USD",amount:0,description:draft.work.description||draft.work.title||"Work Order",status:draft.work.status,documentNumber:draft.identity.number,occurredAt:`${draft.dates.performedOn}T12:00:00.000Z`,workOrder:draft,references:context.references||[]},commandId:resolvedCommandId,idempotencyKey:clean(idempotencyKey)||resolvedCommandId,metadata:{...obj(metadata),transactModule:"work-order",workOrderSchema:draft.schema,workPerformedOn:draft.dates.performedOn},apiBaseUrl,headers,signal});
