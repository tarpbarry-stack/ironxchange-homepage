...
function refs(context = {}, record = {}) {
  return mergeIXIAosFinancialReferences(
    [
      ...
      clean(record?.customer?.passportId)
        ? {
            passportId: clean(record.customer.passportId),
            role: "customer",
            label: clean(record.customer.name),
            objectType: "entity",
          }
        : null,
    ].filter(Boolean),
...
function canonical(record, result) {
  const envelope =
    result?.record ||
    result?.data?.record ||
    result?.persistence?.data?.record ||
    {};
  const document =
    envelope.financialDocument || result?.financialDocument || {};
  const value = document.salesOrder || record;
  return {
    ...value,
    identity: {
      ...value.identity,
      salesOrderId: document.financialDocumentId,
      financialDocumentId: document.financialDocumentId,
      number: document.documentNumber || value?.identity?.number,
    },
    financialBinding: {
      financialDocumentId: document.financialDocumentId,
      revision: Number(envelope?.server?.revision || 1),
...
function canonicalInvoice(invoice, result) {
  const envelope =
    ...
    {};
  const document =
    envelope.financialDocument || result?.financialDocument || invoice || {};
  return {
    ...document,
    financialBinding: {
      financialDocumentId: clean(document.financialDocumentId),
      revision: Number(
        envelope?.server?.revision || invoice?.financialBinding?.revision || 1,
      ),
      line: document?.lines?.[0] || null,
    },
  };
...
function invoiceMetadata(record = {}, input = {}, invoice = {}) {
  const salesOrderId = clean(
    invoice?.sourceFinancialDocumentId ||
      invoice?.metadata?.salesOrderId ||
...
export async function saveIXIEquipmentSale({
  object = {},
  context = {},
  record = {},
  action = "save",
  ...
  if (id) {
    const response = await patchIXIAosFinancialDocument({
      ...
      expectedRevision: Number(record?.financialBinding?.revision),
      commandId: crypto.randomUUID(),
      idempotencyKey: `ixi-sales-order:${action}:${crypto.randomUUID()}`,
      patch: {
        salesOrder: stored(record),
        financialState: "committed",
        attachments: record?.termsDocument?.url
          ? [
              {
                attachmentId: record.termsDocument.documentId,
                type: "terms-and-conditions",
                url: record.termsDocument.url,
                sha256: record.termsDocument.sha256,
                pageCount: 2,
              },
            ]
          : [],
  ...
  const commandId =
    clean(record?.identity?.clientRequestId) || crypto.randomUUID();
  const response = await createIXIAosObjectFinancialDocument({
    object,
    documentType: "sales-order",
    commandId,
    idempotencyKey: `ixi-sales-order:${commandId}`,
    signal,
    input: {
      financialState: "committed",
      ...
      description: `${record?.dealType === "rental-purchase-option" ? "RPO" : "Equipment"} Sales Order · ${clean(record?.customer?.name)} · ${clean(record?.asset?.label)}`,
      salesOrder: stored(record),
      references,
      attachments: record?.termsDocument?.url
        ? [
        ...
        : [],
      sourceFinancialDocumentId: clean(record?.related?.quoteId),
    },
    ...
    metadata: {
      ...
      dealId: clean(record?.identity?.dealId),
      dealType: record?.dealType || "standard-sale",
      salesOrderStatus: record.status,
    },
  });
  return { response, record: canonical(record, response) };
}

export async function createIXIEquipmentSaleSigningInvitation(
  record,
  { expiresInHours = 168 } = {},
) {
  const id = clean(record?.financialBinding?.financialDocumentId);
  if (!id)
    throw new Error("Save the Sales Order before sending it for signature.");
  const response = await fetch(
    `/api/ixi/financial/sales-orders/${encodeURIComponent(id)}/signing-invitations`,
    {
      method: "POST",
      credentials: "include",
      headers: {
      ...
      body: JSON.stringify({
        expiresInHours,
        commandId: crypto.randomUUID(),
        idempotencyKey: `ixi-sales-order-invite:${id}:${record?.financialBinding?.revision}`,
      }),
    },
  );
  ...
  if (!response.ok || payload?.ok !== true)
    throw new Error(
      payload?.errors?.[0]?.message ||
        "Signing invitation could not be created.",
    );
  return payload.data;
}

function workflowPayload(payload = {}, fallbackRecord = {}) {
  const envelope = payload?.data?.record || {};
  const document = envelope?.financialDocument || {};
  const order = document?.salesOrder || payload?.data?.order || fallbackRecord;
  return {
    order: {
      ...order,
      financialBinding: {
        financialDocumentId: clean(
          document.financialDocumentId ||
            fallbackRecord?.financialBinding?.financialDocumentId,
        ),
        revision: Number(
          envelope?.server?.revision ||
            fallbackRecord?.financialBinding?.revision ||
            1,
        ),
        line:
          document?.lines?.[0] ||
          fallbackRecord?.financialBinding?.line ||
          null,
      },
    },
...
async function postSalesOrderWorkflow(record = {}, operation = "", input = {}) {
  const id = clean(
    record?.financialBinding?.financialDocumentId ||
      record?.identity?.salesOrderId,
  );
  if (!id) throw new Error("Save the Sales Order before continuing.");
  const response = await fetch(
    `/api/ixi/financial/sales-orders/${encodeURIComponent(id)}/${operation}`,
    {
      method: "POST",
      credentials: "include",
      headers: {
...
export function ensureIXIEquipmentSaleInvoice(record = {}) {
  return postSalesOrderWorkflow(record, "ensure-invoice", {
    commandId: crypto.randomUUID(),
    idempotencyKey: `ixi-sales-order-invoice:${clean(record?.financialBinding?.financialDocumentId || record?.identity?.salesOrderId)}`,
  });
}

export function attestIXIEquipmentSaleSigned(record = {}, input = {}) {
  const id = clean(
    record?.financialBinding?.financialDocumentId ||
      record?.identity?.salesOrderId,
  );
  return postSalesOrderWorkflow(record, "manual-signature", {
    ...input,
    attestation: input.attestation === true,
    commandId: crypto.randomUUID(),
    idempotencyKey: `ixi-sales-order-manual-sign:${id}:${clean(input.signerDate)}:${clean(input.signerName).toLowerCase()}`,
  });
}

export async function saveIXIEquipmentInvoiceDraft(invoice = {}, input = {}) {
  const id = clean(
    invoice?.financialBinding?.financialDocumentId ||
      invoice?.financialDocumentId,
  );
  const revision = Number(invoice?.financialBinding?.revision || 0);
  if (!id || revision < 1)
    throw new Error("The generated Invoice is not bound to IX Core.");
  if (clean(invoice.financialState).toLowerCase() !== "draft")
    throw new Error(
      "An issued Invoice is immutable. Use a credit or replacement control.",
    );
  ...
  return patchIXIAosFinancialDocument({
    ...
    commandId,
    idempotencyKey: `ixi-equipment-invoice-draft:${commandId}`,
    patch: {
    ...
    metadata: {
      transactModule: "equipment-sale",
      action: "edit-draft-invoice",
    },
  });
}

export async function saveIXIEquipmentInvoice({
  object = {},
  context = {},
  record = {},
  invoice = null,
  ...
  const linkedSalesOrderId = clean(
  ...
  const sourceSalesOrderId = clean(
    linkedSalesOrderId ||
      record?.financialBinding?.financialDocumentId ||
      record?.identity?.salesOrderId,
  );
  ...
  if (id) {
    ...
    const response = await patchIXIAosFinancialDocument({
      ...
      patch: {
        ...commercialPatch,
        documentNumber: clean(invoice?.documentNumber) || invoiceNumber(id),
      ...
      metadata: {
        transactModule: "equipment-sale",
        action: linkedSalesOrderId
          ? "edit-linked-draft-invoice"
          : "edit-direct-draft-invoice",
      },
      signal,
    });
  ...
  const response = await createIXIAosObjectFinancialDocument({
    ...
    metadata: {
      transactModule: "equipment-sale",
      dealId: clean(record?.identity?.dealId),
      action: sourceSalesOrderId
        ? "create-linked-draft-invoice"
...
export async function issueIXIEquipmentInvoice({
  invoice = null,
  signal,
} = {}) {
  const id = clean(
  ...
  const revision = Number(invoice?.financialBinding?.revision || 0);
  if (!id || revision < 1)
    throw new Error("Save the Invoice before issuing it.");
  if (clean(invoice?.financialState).toLowerCase() !== "draft")
    throw new Error("Only a draft Invoice can be issued.");
  if (!clean(invoice?.documentNumber))
    throw new Error("Invoice number is required before issue.");
  const commandId = crypto.randomUUID();
