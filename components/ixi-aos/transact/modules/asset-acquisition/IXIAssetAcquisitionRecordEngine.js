...
export function getIXIAcquisitionOperations(record = {}, transactions = []) {
  ...
  const freightRank = { "not-created": 0, draft: 1, requested: 2, awarded: 3, dispatched: 4, picked_up: 5, "picked-up": 5, delivered: 6, billed: 7, reconciled: 8, paid: 9, closed: 10 };
  for (const item of arr(transactions).map(canonicalDocument)) {
    const refs = arr(item?.references || item?.additionalReferences);
    const related =
      refs.some((ref) => clean(ref?.passportId) === passportId || clean(ref?.externalId) === acquisitionId) ||
      clean(item?.assetPassportId || item?.metadata?.assetPassportId) === passportId ||
      clean(item?.acquisitionId || item?.metadata?.acquisitionId) === acquisitionId;
    if (!related) continue;
    const documentType = clean(item?.documentType || item?.metadata?.transactModule);
    const workType = clean(item?.workOrder?.work?.type || item?.work?.type || item?.metadata?.workType).toLowerCase();
    const status = clean(item?.freightOrder?.status || item?.status || item?.workOrder?.work?.status || item?.work?.status || "open");
    if (documentType === "freight" || clean(item?.metadata?.transactModule) === "freight") {
      freightOrderCount += 1;
      const candidate = status || "created";
      if ((freightRank[candidate] ?? 1) >= (freightRank[freightStatus] ?? 0)) freightStatus = candidate;
    }
    if (documentType === "work-order" && (workType.includes("inspection") || clean(item?.metadata?.acquisitionWorkflow) === "receiving-inspection")) {
      inspectionStatus = ["complete", "completed", "closed"].includes(status) ? "complete" : status || "open";
    }
    if (documentType === "work-order" && (workType.includes("make ready") || workType.includes("make-ready") || clean(item?.metadata?.acquisitionWorkflow) === "make-ready")) {
      makeReadyWorkOrderCount += 1;
      if (!["complete", "completed", "closed", "cancelled", "void"].includes(status)) makeReadyOpenCount += 1;
    }
  }
  return { freightStatus, freightOrderCount, inspectionStatus, makeReadyOpenCount, makeReadyWorkOrderCount };
}

export function amendIXIAssetAcquisition(record = {}, amendment = {}, actor = {}) {
  const field = clean(amendment.field);
  if (!Object.hasOwn(BASIS_FIELD_DIRECTIONS, field)) throw new Error("Select a valid acquisition cost field.");
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(effectiveDate)) throw new Error("A valid amendment effective date is required.");
  if (!reason) throw new Error("An amendment reason is required.");
  if (!reference) throw new Error("An invoice, document, or approval reference is required.");
  if (newValue < 0) throw new Error("Acquisition cost fields cannot be negative.");
  const acquisition = { ...(record.acquisition || {}) };
  const previousValue = round(acquisition[field] ?? (field === "nonrecoverableTax" ? acquisition.tax : 0));
  const basisBefore = round(acquisition.currentAcquisitionBasis ?? acquisition.directAcquisitionCost);
  const basisDelta = round((newValue - previousValue) * BASIS_FIELD_DIRECTIONS[field]);
  const basisAfter = round(basisBefore + basisDelta);
  if (basisAfter < 0) throw new Error("The amended acquisition basis cannot be negative.");
  ...
  return {
    ...
    funding: {
      ...(record.funding || {}),
      balanceDue: round(Math.max(0, basisAfter - num(record?.funding?.amountPaid))),
    },
...
export function normalizeIXIPackageAllocation(record = {}, normalization = {}, actor = {}) {
  ...
  if (!packageId || !packageReference) throw new Error("Package ID and purchase document reference are required.");
  if (!/^\d{4}-\d{2}-\d{2}$/.test(effectiveDate)) throw new Error("A valid normalization effective date is required.");
  ...
  if (!(packageTotal > 0) || allocations.length < 2) throw new Error("Package total and at least two machine allocations are required.");
  if (allocations.some((item) => !item.passportId || item.amount < 0)) throw new Error("Every allocation requires an IXI Passport and a non-negative amount.");
  if (new Set(allocations.map((item) => item.passportId)).size !== allocations.length) throw new Error("Package allocations require unique IXI Passports.");
  const allocatedTotal = round(allocations.reduce((sum, item) => sum + item.amount, 0));
  if (Math.abs(allocatedTotal - packageTotal) > 0.005) throw new Error("Package allocations must equal the authoritative package total.");
  const passportId = clean(record?.context?.primaryPassportId);
  const current = allocations.find((item) => item.passportId === passportId);
  if (!current) throw new Error("This acquisition's IXI Passport must be included in the package allocation.");
  const acquisition = { ...(record.acquisition || {}) };
  const basisBefore = round(acquisition.currentAcquisitionBasis ?? acquisition.directAcquisitionCost);
  ...
  return {
    ...
    acquisition: {
      ...acquisition,
      packageNormalizationTotal: round(num(acquisition.packageNormalizationTotal) + basisDelta),
      currentAcquisitionBasis: basisAfter,
      directAcquisitionCost: basisAfter,
    },
    funding: {
...
export function applyIXIAcquisitionActuals(record = {}, transactions = []) {
  ...
  const known = new Set(arr(record.adjustments).map((item) => clean(item?.adjustmentId)));
  const packageEvents = arr(transactions)
    .map(canonicalDocument)
    .map((item) => item?.packageNormalization || item?.metadata?.packageNormalization)
    .filter((event) => clean(event?.type) === "package-normalization" && !known.has(clean(event?.adjustmentId)))
    .sort((left, right) => clean(left?.occurredAt).localeCompare(clean(right?.occurredAt)));
  for (const event of packageEvents) {
    const allocation = arr(event?.allocations).find((item) => clean(item?.passportId) === passportId);
    if (!allocation) continue;
    const acquisition = { ...(projectedRecord.acquisition || {}) };
    const basisBefore = round(acquisition.currentAcquisitionBasis ?? acquisition.directAcquisitionCost);
    const basisAfter = round(allocation.amount);
    const projectedEvent = { ...event, basisBefore, basisAfter, basisDelta: round(basisAfter - basisBefore), projectedFromControl: true };
    projectedRecord = {
      ...projectedRecord,
      acquisition: {
        ...acquisition,
        packageNormalizationTotal: round(num(acquisition.packageNormalizationTotal) + projectedEvent.basisDelta),
        currentAcquisitionBasis: basisAfter,
        directAcquisitionCost: basisAfter,
      },
      funding: {
        ...(projectedRecord.funding || {}),
        balanceDue: round(Math.max(0, basisAfter - num(projectedRecord?.funding?.amountPaid))),
      },
      packageAllocation: {
  ...
  const estimated = round(projectedRecord?.makeReady?.estimates?.reduce((sum, item) => sum + num(item?.estimatedAmount), 0));
  return {
    ...projectedRecord,
    makeReady: {
