...
export default function IXITransactApp({ object = {}, actor = {}, entity = {}, activeWorkOrder = null, activeTechWorkOrder = null, permissions = [], financialRecords = [], onFinancialRecordsChange = null, onClose = null, onOpenModule = null, onSendFront = null, onSendBack = null, onCycleColor = null, onCycleOutline = null, armedDestination = "", onSendToArmedDestination = null }) {
  ...
  else if (moduleId === "payables") body = <IXIPayablesApp context={context} object={object} financialRecords={object?.payableFinancialRecords || object?.assetFinancialTransactions || object?.relatedFinancialRecords || object?.financialRecords || []} initialCases={payableCases} onBack={back} onRecordChange={async (record, changePayload, sourceContext) => { if (record?.payable?.billId) setPayableCases(current => [...current.filter(item => item.payable?.billId !== record.payable.billId), record]); await change("payables", "PAYABLES UPDATE", "pay", changePayload?.action === "payment" ? "payment" : changePayload?.action === "credit" ? "credit" : "payables-control", "payableCase", record, changePayload, sourceContext || context, { payable: record?.payable || null, financialResponse: changePayload?.financialResponse || null }); }} />;
  else if (moduleId === "treasury") body = <IXITreasuryApp context={context} object={object} financialRecords={object?.treasuryFinancialRecords || object?.assetFinancialTransactions || object?.relatedFinancialRecords || object?.financialRecords || []} initialAccounts={treasuryAccounts} initialReconciliations={treasuryReconciliations} expectedInflows={object?.treasuryExpectedInflows || []} scheduledOutflows={object?.treasuryScheduledOutflows || []} onBack={back} onRecordChange={async (record, changePayload, sourceContext) => { if (record?.identity?.accountId) setTreasuryAccounts(current => [...current.filter(item => item.identity?.accountId !== record.identity.accountId), record]); if (changePayload?.reconciliation?.identity?.reconciliationId) setTreasuryReconciliations(current => [...current, changePayload.reconciliation]); await change("treasury", "CASH / TREASURY UPDATE", "cash", changePayload?.action === "reconciliation" ? "reconciliation" : "payment", "treasuryAccount", record, changePayload, sourceContext || context, { reconciliation: changePayload?.reconciliation || null, financialResponse: changePayload?.financialResponse || null, adjustment: changePayload?.adjustment || null, transfer: changePayload?.transfer || null }); }} />;
