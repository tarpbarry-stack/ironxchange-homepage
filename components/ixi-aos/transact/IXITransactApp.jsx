...
export default function IXITransactApp({ object = {}, actor = {}, entity = {}, activeWorkOrder = null, permissions = [], onClose = null, onOpenModule = null, onSendFront = null, onSendBack = null, onCycleColor = null, onCycleOutline = null, armedDestination = "", onSendToArmedDestination = null }) {
  ...
  else if (moduleId === "service-invoice") body = <IXIServiceInvoiceApp context={context} object={object} workOrder={workOrderSnapshot || activeWorkOrder} onBack={back} onRecordChange={async (record, changePayload, sourceContext) => { const currentWorkOrder = workOrderSnapshot || activeWorkOrder || {}; const nextWorkOrder = { ...currentWorkOrder, references: { ...(currentWorkOrder.references || {}), serviceInvoiceIds: [...new Set([...(currentWorkOrder.references?.serviceInvoiceIds || []), clean(record?.identity?.serviceInvoiceId || record?.identity?.number)].filter(Boolean))] }, financial: { ...(currentWorkOrder.financial || {}), invoicedRevenue: Number(record?.charges?.amountDue || currentWorkOrder.financial?.invoicedRevenue || 0), receivedRevenue: Number(record?.ar?.amountReceived || currentWorkOrder.financial?.receivedRevenue || 0) } }; setWorkOrderSnapshot(nextWorkOrder); await change("service-invoice", "SERVICE INVOICE UPDATE", "sell", changePayload?.action === "record-payment" ? "payment" : "invoice", "serviceInvoice", record, changePayload, sourceContext || context, { customerServiceWorkOrder: nextWorkOrder, workOrder: nextWorkOrder, customer: record?.customer || null, ar: record?.ar || null }); }} />;
  else if (moduleId === "work-order") body = <IXIWorkOrderApp context={context} initialWorkOrder={workOrderSnapshot || activeWorkOrder} onBack={back} onCreate={async (record, sourceContext) => { setWorkOrderSnapshot(record); await onOpenModule?.({ id: "work-order-create", label: "CREATE WORK ORDER", group: "work", documentType: "work-order" }, sourceContext, { workOrder: record }); }} onAction={async (id, workOrder, sourceContext, payload = {}) => { let nextWorkOrder = workOrder; if (id === "complete") nextWorkOrder = { ...(workOrder || {}), work: { ...(workOrder?.work || {}), status: "complete" }, financial: { ...(workOrder?.financial || {}), status: "complete" } }; setWorkOrderSnapshot(nextWorkOrder); await onOpenModule?.({ id, label: String(id).toUpperCase(), group: "work-order-action", documentType: id }, sourceContext, { workOrder: nextWorkOrder, ...payload }); if (id === "complete" && nextWorkOrder?.work?.customerService) setModuleId("service-invoice"); }} />;
