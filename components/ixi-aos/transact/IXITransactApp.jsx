...
function addUnique(values = [], id = "") { const key = clean(id); if (!key) return Array.isArray(values) ? values : []; return [...new Set([...(Array.isArray(values) ? values : []), key])]; }
function appendUniqueRecord(records = [], record = {}, identityGetter = () => "") { const list = Array.isArray(records) ? records : []; const identity = clean(identityGetter(record)); if (!identity) return [...list, record]; const existingIndex = list.findIndex(item => clean(identityGetter(item)) === identity); if (existingIndex < 0) return [...list, record]; return list.map((item, index) => index === existingIndex ? record : item); }
...
export default function IXITransactApp({ object = {}, actor = {}, entity = {}, activeWorkOrder = null, permissions = [], onClose = null, onOpenModule = null, onSendFront = null, onSendBack = null, onCycleColor = null, onCycleOutline = null, armedDestination = "", onSendToArmedDestination = null }) {
  ...
  const compactHeader = Boolean(active) || noteOpen || photoOpen || expenseOpen || purchaseOrderOpen || timeOpen || materialOpen || assetAcquisitionOpen;
  ...
  async function workOrderAction(actionId, workOrder, workContext, payload = {}) { const nextWorkOrder = workOrder || resolvedWorkOrder || null; if (nextWorkOrder) setWorkOrderSnapshot(nextWorkOrder); if (actionId === "note") { setModuleId("work-order-note"); return; } if (actionId === "photo") { setModuleId("work-order-photo"); return; } await onOpenModule?.({ id: actionId, label: String(actionId || "").toUpperCase(), group: "work-order-action", documentType: actionId }, workContext, { workOrder: nextWorkOrder, ...payload }); }

  async function saveDirectExpense(expense, input, response) {
    const expenseId = clean(expense?.identity?.expenseId || expense?.identity?.number || expense?.identity?.clientRequestId); if (!expenseId) throw new Error("Saved Expense did not return a stable identity.");
    const activity = { activityId: `ACT-${expenseId}`, type: "expense-recorded", expenseId, amount: Number(expense?.expense?.amount || 0), vendor: clean(expense?.expense?.vendor), paymentMethod: clean(expense?.expense?.paymentMethod), reimbursementRequired: Boolean(expense?.reimbursement?.required), occurredAt: new Date().toISOString(), actorLabel: clean(context.actor?.displayName || context.actor?.name || context.actor?.label) };
    await onOpenModule?.({ id: "expense-save", label: "SAVE EXPENSE", group: "spend", documentType: "expense" }, context, { expense: { ...expense, identity: { ...(expense?.identity || {}), expenseId } }, input, response, activity, reimbursement: expense?.reimbursement || null, returnTo: "transact-origin" }); setModuleId("");
  }
  async function purchaseOrderChanged(nextRecord, change = {}) { await onOpenModule?.({ id: `purchase-order-${change.action || "change"}`, label: "PURCHASE ORDER UPDATE", group: "buy", documentType: "purchase-order" }, context, { purchaseOrderRecord: nextRecord, change, returnTo: "purchase-order" }); }
  async function billChanged(nextRecord, change = {}) { await onOpenModule?.({ id: `bill-${change.action || "change"}`, label: "BILL / INVOICE UPDATE", group: "spend", documentType: change.action === "record-payment" ? "payment" : "bill" }, context, { billRecord: nextRecord, change, returnTo: "bill" }); }
  async function timeChanged(nextRecord, change = {}, timeContext = context) { await onOpenModule?.({ id: `time-${change.action || "change"}`, label: "TIME UPDATE", group: "work", documentType: "time-entry" }, timeContext, { timeRecord: nextRecord, change, originatingObject: timeContext.primary, returnTo: "time" }); }
  ...
  async function saveNote(note, input, response) {
    const noteId = clean(note?.identity?.noteId || note?.identity?.clientRequestId); if (!noteId) throw new Error("Saved Note did not return a stable identity.");
    const attachment = note?.note?.attachments?.[0] || null; const current = resolvedWorkOrder || {}; const storedNote = { ...note, identity: { ...(note.identity || {}), noteId } };
    const activity = { activityId: `ACT-${noteId}`, type: "note-added", noteId, noteType: note?.note?.type || "work-note", title: note?.note?.title || "", body: note?.note?.body || "", occurredAt: note?.audit?.createdAt || new Date().toISOString(), actorLabel: note?.audit?.createdByLabel || clean(context.actor?.displayName || context.actor?.name || context.actor?.label) };
    let documentProjection = Array.isArray(current.documentProjection) ? current.documentProjection : [];
    if (attachment) { const attachmentIdentity = clean(attachment.documentId || attachment.id) || `NOTE-ATTACH:${noteId}:${attachment.fileName || "attachment"}`; const document = { documentId: attachmentIdentity, title: attachment.fileName || "Note attachment", fileName: attachment.fileName || "", type: attachment.mimeType?.startsWith("image/") ? "photo" : "other", issuer: note?.audit?.createdByLabel || "", relatedType: "note", relatedId: noteId, relatedLabel: note?.note?.title || noteId, date: note?.audit?.createdAt || new Date().toISOString(), addedBy: note?.audit?.createdByLabel || "", mimeType: attachment.mimeType || "", size: Number(attachment.size || 0), persistenceState: attachment.status || "local-pending-upload" }; documentProjection = appendUniqueRecord(documentProjection, document, item => item?.documentId || item?.id); }
    const next = { ...current, references: { ...(current.references || {}), noteIds: addUnique(current.references?.noteIds, noteId) }, notesProjection: appendUniqueRecord(current.notesProjection, storedNote, item => item?.identity?.noteId || item?.identity?.clientRequestId), activityProjection: appendUniqueRecord(current.activityProjection, activity, item => item?.activityId), documentProjection };
    setWorkOrderSnapshot(next); await onOpenModule?.({ id: "note-save", label: "SAVE NOTE", group: "work-order-action", documentType: "note" }, context, { workOrder: next, note: storedNote, input, response, activity }); setModuleId("work-order");
  }

  async function savePhoto(photo, input, response) {
    const photoId = clean(photo?.identity?.photoId || photo?.identity?.clientRequestId); if (!photoId) throw new Error("Saved Photo entry did not return a stable identity.");
    const current = resolvedWorkOrder || {}; const media = Array.isArray(photo?.photo?.media) ? photo.photo.media : []; const attachmentIds = media.reduce((ids, item) => addUnique(ids, item.mediaId), current.references?.attachmentIds || []);
    ...
    let documentProjection = Array.isArray(current.documentProjection) ? current.documentProjection : []; for (const document of documents) documentProjection = appendUniqueRecord(documentProjection, document, item => item?.documentId || item?.id);
    const storedPhoto = { ...photo, identity: { ...(photo.identity || {}), photoId } }; const activity = { activityId: `ACT-${photoId}`, type: "photo-added", photoId, photoType: photo?.photo?.type || "work-photo", title: photo?.photo?.title || "", count: media.length, occurredAt: photo?.photo?.occurredAt || new Date().toISOString(), actorLabel: photo?.audit?.createdByLabel || clean(context.actor?.displayName || context.actor?.name || context.actor?.label) };
    const next = { ...current, references: { ...(current.references || {}), photoIds: addUnique(current.references?.photoIds, photoId), attachmentIds }, photoProjection: appendUniqueRecord(current.photoProjection, storedPhoto, item => item?.identity?.photoId || item?.identity?.clientRequestId), documentProjection, activityProjection: appendUniqueRecord(current.activityProjection, activity, item => item?.activityId) };
    setWorkOrderSnapshot(next); await onOpenModule?.({ id: "photo-save", label: "SAVE PHOTO", group: "work-order-action", documentType: "photo" }, context, { workOrder: next, photo: storedPhoto, input, response, activity, documents }); setModuleId("work-order");
  }

  if (billOpen) return <IXIBillStandaloneApp context={context} object={object} authority={actor?.billAuthority || actor?.financialAuthority || actor?.purchasingAuthority || {}} onBack={() => setModuleId("")} onRecordChange={billChanged} />;
  ...
  return <div className={`ixi-transact-app ixi-transact-v13 board-color-none board-outline-1 ${compactHeader ? "module-open" : "home-open"}`}>
    ...
    <main className="tx-body">
      ...
      : purchaseOrderOpen ? <IXIPurchaseOrderApp context={context} onBack={() => setModuleId("")} onRecordChange={purchaseOrderChanged} />
      : timeOpen ? <IXITimeStandaloneApp context={context} object={object} onBack={() => setModuleId("")} onRecordChange={timeChanged} />
