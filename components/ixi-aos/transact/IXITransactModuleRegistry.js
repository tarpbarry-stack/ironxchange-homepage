...
const MACHINE_ORDER=Object.freeze(["work-order","expense","technology-work","time","material","asset-acquisition","rental-expense","rental-income","purchase-order","receipt","bill","quote","invoice","settlement"]);const LOCATION_ORDER=Object.freeze(["work-order","expense","purchase-order","bill","receipt","technology-work","time","material","asset-acquisition","rental-expense","rental-income","quote","invoice","settlement"]);function sortByOrder(items,order){const rank=new Map(order.map((id,index)=>[id,index]));return[...items].sort((l,r)=>(rank.has(l.id)?rank.get(l.id):999)-(rank.has(r.id)?rank.get(r.id):999));}function deniedModuleIds(permissions=[]){return new Set((Array.isArray(permissions)?permissions:[]).map(v=>String(v||"").trim()).filter(v=>v.startsWith("deny:")).map(v=>v.slice(5)).filter(Boolean));}export function getIXITransactModules({objectType="",permissions=[]}={}){const type=String(objectType||"").trim().toLowerCase(),denied=deniedModuleIds(permissions);let preferred=IXI_TRANSACT_MODULES;if(["machine","equipment","vehicle","truck","trailer"].includes(type))preferred=sortByOrder(IXI_TRANSACT_MODULES,MACHINE_ORDER);else if(["location","yard","shop"].includes(type))preferred=sortByOrder(IXI_TRANSACT_MODULES,LOCATION_ORDER);return preferred.filter(item=>!denied.has(item.id));}export function getIXITransactModule(moduleId=""){const id=String(moduleId||"").trim();return IXI_TRANSACT_MODULES.find(item=>item.id===id)||null;}export default{getIXITransactModules,getIXITransactModule};