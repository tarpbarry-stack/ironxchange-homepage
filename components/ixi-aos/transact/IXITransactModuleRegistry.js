...
export function getIXITransactModules({objectType="",permissions=[]}={}){
  ...
  if(["machine","equipment","vehicle","truck","trailer"].includes(type)){
    const order=["work-order","technology-work","time","material","expense","purchase-order","receipt","bill","quote","invoice","settlement"];
    preferred=[...IXI_TRANSACT_MODULES].sort((a,b)=>order.indexOf(a.id)-order.indexOf(b.id));
  } else if(["location","yard","shop"].includes(type)){
    const order=["work-order","expense","purchase-order","bill","receipt","technology-work","time","material","quote","invoice","settlement"];
    preferred=[...IXI_TRANSACT_MODULES].sort((a,b)=>order.indexOf(a.id)-order.indexOf(b.id));
  }
