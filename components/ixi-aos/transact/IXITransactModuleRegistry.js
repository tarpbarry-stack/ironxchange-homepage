...
const MACHINE_ORDER = Object.freeze(["work-order","expense","technology-work","time","material","asset-acquisition","rental-expense","rental-income","service-quote","service-invoice","purchase-order","receipt","bill","quote","invoice","settlement"]);
const LOCATION_ORDER = Object.freeze(["work-order","expense","purchase-order","bill","receipt","technology-work","time","material","asset-acquisition","rental-expense","rental-income","service-quote","service-invoice","quote","invoice","settlement"]);

function sortByOrder(items, order) { const rank = new Map(order.map((id,index)=>[id,index])); return [...items].sort((left,right)=>(rank.has(left.id)?rank.get(left.id):999)-(rank.has(right.id)?rank.get(right.id):999)); }
function deniedModuleIds(permissions = []) { return new Set((Array.isArray(permissions)?permissions:[]).map(value=>String(value||"").trim()).filter(value=>value.startsWith("deny:")).map(value=>value.slice(5)).filter(Boolean)); }
