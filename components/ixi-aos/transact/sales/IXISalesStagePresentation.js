...
export function isIXISalesStageCompleted(deal, stageId) {
  ...
  if (stageId === "signed")
    return Boolean(records.signed || invoiceCarriesVerifiedSignature(deal));
  // The rail shows durable document progression. A linked draft Invoice is a
  // real, openable document even though it does not unlock SOLD until issued.
  if (stageId === "invoice")
    return Boolean(records.invoice || records.sold || records.settlement);
...
export function canStartIXISalesStage(deal, stageId) {
  if (stageId === "quote")
    return Boolean(
      deal?.stageRecords?.["sales-order"] || deal?.stageRecords?.invoice,
    );
  if (stageId === "sales-order")
    return Boolean(deal?.stageRecords?.quote || deal?.stageRecords?.invoice);
  if (stageId === "signed") return Boolean(deal?.stageRecords?.["sales-order"]);
  if (stageId === "invoice")
    return Boolean(
      deal?.stageRecords?.["sales-order"] || deal?.stageRecords?.signed,
    );
...
export function salesStagePresentation(deal, stageId, activeStageId = "") {
  ...
  if (completed)
    return { entry, completed: true, startable, selected, state: "completed" };
  if (startable && selected)
    return { entry: null, startable: true, selected: true, state: "next" };
  if (startable)
    return {
      entry: null,
      completed: false,
