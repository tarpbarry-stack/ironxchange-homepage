...
const page = fs.readFileSync("pages/aos/mobile-console-left.js", "utf8");
...
test("mobile left Console reuses the canonical private Console and immutable scaler", () => {
...
test("left Console gate is one machine plus one module only", () => {
  assert.match(page, /const nativeWidth = consoleDepth === 2 \? 600 : 300/);
  assert.match(page, /consoleSlots\.length > 2/);
  assert.match(page, /consoleSlots: nextPatch\.consoleSlots\.slice\(-2\)/);
  assert.match(page, /ixi-private-console-listing-slot \.ixi-object-card-actuator\.right\{display:none!important\}/);
  assert.match(page, /ixi-private-console-module-slot \.ixi-object-card-actuator\.left\{display:none!important\}/);
});
