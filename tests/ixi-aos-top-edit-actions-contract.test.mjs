...
test("Cards 001-018 use the commercial editor header actions", () => {
...
test("the shared repair covers every numbered card from 001 through 018", () => {
  ...
  for (let number = 1; number <= 18; number += 1) {
    ...
    if (number === 18) {
      assert.match(source, /cardNumber=\{cardDefinition\.cardNumber\}/u, "Card 018 must publish the selected card number");
    } else {
...
test("007A, 007B, and 007C remain on the shared Card 007 edit contract", () => {
  ...
  assert.match(card007, /selectedVariant === "B"/u);
  assert.match(card007, /selectedVariant === "C"/u);
