...
test("one canonical Object produces one command context and keeps machine presentation", () => {
  ...
  assert.equal(matching[0].stockNumber, "SSU-544K-01");
...
test("recursive command center owns transact while the detailed ledger remains available", () => {
  ...
  // Building a launch context or in-memory read cache does not provision an Object or Passport.
  // Financial writes remain in the shared authenticated payment commands.
  assert.doesNotMatch(commandCenter, /createIXI(?!Transact(?:Context|RecordCache|HistoryCache)\b)|provision|passport\/ensure/u);
