...
test("placement identity is tenant/workspace/session/object/scope keyed", () => {
  ...
  assert.deepEqual(getAosPlacementIdentity(personal, "object_ripper"), {
    tenantId: "entity-star-and-sons",
    workspaceId: "aos-work",
    sessionId: "session-001",
    objectId: "object_ripper",
    placementScope: {
      kind: "personal",
      scopeId: "personal:user-001",
      ownerActorId: "user-001"
...
test("first admission captures immutable origin for previews and later loads", () => {
  let state = admit(personalSession(), "object_ripper", "rail:equipment", T1, {
    activeSummonedContext: "object_equipment-index",
    visualOrder: 4,
    admissionMode: "preview"
  });
  const origin = state.objectsById["object_ripper"].sessionOrigin;

  const replay = admit(state, "object_ripper", "board", T2, {
    activeSummonedContext: "object_wichita-falls",
    visualOrder: 0,
    admissionMode: "operating"
  });

  assert.equal(replay, state);
  assert.deepEqual(replay.objectsById["object_ripper"].sessionOrigin, origin);
  assert.equal(replay.objectsById["object_ripper"].operatingState, "preview");
});

test("move and summoned context change session state without touching origin", () => {
  const admitted = admit(personalSession(), "object_ripper", "rail:equipment");
  ...
  assert.equal(admitted.objectsById["object_ripper"].currentSurface, "rail:equipment");
  assert.equal(recalledContext.objectsById["object_ripper"].currentSurface, "surface:wichita-falls");
  assert.equal(recalledContext.objectsById["object_ripper"].activeSummonedContext, "object_equipment-index");
  assert.equal(recalledContext.objectsById["object_ripper"].sessionOrigin.surfaceId, "rail:equipment");
...
test("operation undo restores its snapshot while recall-to-start uses origin", () => {
  let state = admit(personalSession(), "object_ripper", "rail:equipment");
  ...
  assert.equal(undone.objectsById["object_ripper"].currentSurface, "board");
  assert.equal(undone.objectsById["object_ripper"].visualOrder, 3);
  assert.equal(undone.objectsById["object_ripper"].returnSnapshot, null);
  ...
  assert.equal(recalled.objectsById["object_ripper"].currentSurface, "rail:equipment");
  assert.equal(recalled.objectsById["object_ripper"].operatingState, "preview");
});

test("sorting changes visual order only", () => {
  let state = personalSession();
  state = admit(state, "object_a", "board", T1, { admissionMode: "operating" });
  state = admit(state, "object_b", "board", T2, { admissionMode: "operating" });
  ...
  const sorted = reorderAosSessionSurface(state, {
    surfaceId: "board",
    orderedObjectIds: ["object_b", "object_a"],
    actorId: "user-001",
    now: T3
  });

  assert.equal(sorted.objectsById["object_b"].visualOrder, 0);
  assert.equal(sorted.objectsById["object_a"].visualOrder, 1);
...
test("persistence round-trip survives refresh and rejects scope crossover", () => {
  const state = admit(personalSession(), "object_ripper", "rail:equipment");
...
test("persistence cannot guess the authoritative revision", () => {
  const state = admit(personalSession(), "object_ripper", "board");
  assert.throws(
...
test("personal scope rejects another employee and shared scope requires server decision", () => {
  ...
  assert.equal(admitted.objectsById["object_ripper"].currentSurface, "board");
...
test("an operation retry cannot overwrite its first undo snapshot", () => {
  ...
  const firstSnapshot = state.objectsById["object_ripper"].returnSnapshot;
  state = moveAosSessionObject(state, {
    objectId: "object_ripper",
    currentSurface: "board",
    actorId: "user-001",
  ...
  assert.deepEqual(replay.objectsById["object_ripper"].returnSnapshot, firstSnapshot);
});
...
test("end clears abandoned snapshots and expiration blocks movement", () => {
  let state = admit(personalSession(), "object_ripper", "board");
  state = captureAosOperationUndo(state, {
    operationId: "operation-001",
    objectIds: ["object_ripper"],
    actorId: "user-001",
  ...
  assert.equal(ended.objectsById["object_ripper"].returnSnapshot, null);
...
test("validation rejects non-canonical object-key structure", () => {
  ...
  corrupted.objectsById["listing-alias"] = corrupted.objectsById["object_ripper"];
  delete corrupted.objectsById["object_ripper"];
